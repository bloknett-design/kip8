...
---
Task ID: 179→190 (перенос, историческая справка)
Agent: AI Assistant (GLM)
Task: Перенос из kip8test: v373-v392 — Tasks 160-179 (kипia-v392) — запись восстановлена задним числом

Work Log:
- Коммит 49d86fe «Перенос из kip8test: v373-v392 — Tasks 160-179
  (kipia-v392)» от 2026-08-26: LED-кнопка КИПиА в десктоп-баре
  (Task 159), счётчики расходомеров в десктопных табах (Task 161),
  перетаскиваемая граница панелей (Task 162), поиск на вложенных
  страницах КИП ИОС (Tasks 164-167), таблица приборов
  devices-table-desktop.js (Tasks 163, 168-176), трёхколоночная схема
  конвертеров и калькуляторов (Tasks 177-179)
- Запись в worklog kip8 тогда не была сделана — восстановлена в
  рамках Task 190

---
Task ID: 190
Agent: AI Assistant (GLM)
Task: Перенос из kip8test: v446-v455 — Tasks 180-189 (kipia-v393) + обновление системного промта (PAT передан пользователем)

Work Log:
- Перенос выполнен скриптом scripts/prepare-kip8-transfer.py (коммитится
  в kip8test): блок isolateLocalStorage() удалён, префиксы
  'kip8test:*' заменены на ключи kip8_*
- kip8@672fa8d (kipia-v392 → kipia-v393):
  - index.html: правки Task 180 (поиск у правого края бара крошек —
    flex:1 1 0% контента), Task 181 (шеврон в строке крошек на
    flowmeter-data: detailBarChevronClick, setFlowmeterBreadcrumbContent,
    updateChevronArrows обновляет оба шеврона, восстановление крошек
    после закрытия панели), Tasks 182-187 (три вертикальные точки на
    разделителе панелей, центрированы на линии, ~3.8px шаг 10px),
    Task 188 (мобильная — убрана кнопка поиска в «Избранном»),
    Task 189 (выравнивание шапки «Избранного» на мобильном +
    секретные кнопки «Сапёр»/«Телефонный справочник» по габаритам
    остальных кнопок главной, у «Сапёра» подзаголовок)
  - tests/test-role-access.js: +10 тестов (Tasks 180/181), 488 → 498
  - Дифф после трансформации = ровно 294 строки изменений Tasks 180-189
    (проверено пофрагментно), никаких тест-специфичных артефактов
- Проверки: 498 passed, 0 failed; node --check 4 script-блоков + sw.js;
  isolateLocalStorage = 0, упоминаний kip8test = 0
- Деплой подтверждён: живой kip8 отдаёт kipia-v393; автосинк в
  kip8-desktop — e95bc37 (CI Tests: success, Build Desktop App: success)
- Системный промт kip8 обновлён до post-Task 189: версии, 498 тестов,
  раздел «Десктоп: строка крошек и разделитель панелей (Tasks 180-189)»

Stage Summary:
- kip8 синхронен с kip8test@16d466b (v455): все правки Tasks 180-189 в бою
...
- Локальная дата: 2026-08-30 (Asia/Novosibirsk, UTC+07:00).
---
Task ID: 258 (порт из kip8test)
Agent: main (Super Z)
Task: Перенос партии Tasks 254-258 из kip8test@d695de1 в боевой kip8:
      визуальная разметка шахматки (розовые выходные, красные
      границы, должности с режимом занятости, прямой тулбар),
      авто-подгонка под окно, полоса-бордюрчик 5px, скрытие полос
      прокрутки. Один коммит = один инкремент SW (kipia-v403).

Work Log:
- Перенесены 3 файла: index.html (патч 4a6da97..d695de1 применился
  чисто — Changes только в модуле WorkSchedule, репо-специфика не
  затронута: ключи kip8_*, isolateLocalStorage отсутствует,
  WEB_APP_URL боевой, регистрация './sw.js' Task 253 жива),
  tests/test-work-schedule.js (адаптация scripts/task258-port-tests.py:
  UI-тесты Tasks 254-258 как в kip8test; SW-тест v402 → история;
  новая секция «Task 258: SW версия v403» с защитой от тестовых
  версий v512-v516 в боевом sw.js), sw.js (kipia-v402 → v403).
- Тесты: 872 → 910 passed / 0 failed (node tests/run-all.js).
- Верификация браузерная (харнесс build-task258-kip8-test.js из
  РЕАЛЬНОГО kip8/index.html): 1280×800 тёмная/светлая, 1280×620,
  1024×768×15 сотр., 1280×740×20 сотр. (tight) — 17/17 проверок в
  каждой: полоса ровно 5.00px, целые высоты строк, нет скроллбаров,
  дробный sweep без переливания, розовые выходные/красные границы/
  подписи должностей живы. Скриншот: scripts/task258-kip8-dark.png.
- Промт обновлён: v403, 910 тестов, post-Task 258 (запись о переносе
  партии + таблица версий kip8 v403 / kip8test v516).
- Коммит 6569afd + push origin/main (c7668eb..6569afd, PAT-протокол,
  URL сброшен, утечек нет).

Stage Summary:
- Task 258 (порт) выполнен и запушен в боевой kip8 (6569afd,
  kipia-v403, 910 passed): в «Графике работы» теперь вся партия
  Tasks 254-258 — розовые пустые выходные, тонкие красные границы
  вне шапки, должности с режимом занятости («Слесарь КИПиА смена
  №1»/«дневной»), непрозрачный тулбар с прямыми углами,
  авто-подгонка под размер окна без полос прокрутки, завершающая
  полоса-бордюрчик 5px.
- Боевой сайт https://bloknett-design.github.io/kip8/ — пользователям
  нужно обновление PWA (кэш v403).
- Серверная часть (Apps Script) не затронута — только фронтенд;
  scripts/WorkSchedule.gs идентичен (в Tasks 254-258 не менялся).
- Локальная дата: 2026-08-30 (Asia/Novosibirsk, UTC+07:00).

---
Task ID: 262 (порт из kip8test)
Agent: main (Super Z)
Task: Перенос партии Tasks 259-262 из kip8test@aad96f4 в боевой kip8:
      бордюрчик с эффектом выступа + группировка/сортировка
      сотрудников (Task 259); производственный календарь РФ —
      legalic основной источник, isDayOff резерв, production-calendar
      токен-опция, офлайн Сб/Вс + ст. 112; День шахтёра Кузбасса
      (Tasks 260/262). Один коммит = один инкремент SW (kipia-v404).

Work Log:
- kip8 склонирован свежо (PAT-протокол, URL сброшен сразу);
  базовая линия проверена: HEAD 70326ad (= 6569afd + docs),
  CACHE_VERSION kipia-v403, рабочее дерево чисто.
- index.html: патч kip8test d695de1..aad96f4 (1740 строк, Tasks
  259/260/262: CSS bevel-бордюрчика, _sortEmployees, ws-group-first,
  модуль ProdCalendar ~1600 строк, интеграция _calDayOff/_renderCell/
  тултипов/чипа/шторки) применился ЧИСТО (git apply). Репо-специфика
  не затронута: ключи ws_pcal_* универсальные (изоляция в kip8test
  runtime-обёртка, в kip8 префиксов нет), isolateLocalStorage
  отсутствует, WEB_APP_URL боевой, регистрация './sw.js' жива;
  скан патча на kip8test/kipia-test/bloknett — 0 вхождений.
  36 279 → 37 831 строк.
- sw.js: kipia-v403 → v404, комментарий партии (по образцу Task
  258-порта: одна партия = один инкремент; тестовые v517-v519 в
  kip8 не существовали). Структурно sw.js обоих репо идентичен
  (проверено нормализованным diff — только шапка/версии).
- tests/test-work-schedule.js: база — текущий файл kip8test
  (функциональные тесты Tasks 259/260/262 идентичны, т.к. index.html
  после патча одинаков); адаптация scripts/task262-port-tests.py
  (каждая замена по счётчику вхождений, при расхождении — отказ):
  исторические заметки Tasks 249/250/252 → формулировки kip8 о
  переносах; 6 SW-секций kip8test (v513/v514/v515/v516/v517/v518,
  9 тестов) удалены; добавлена секция «Task 262: SW версия v404
  (перенос Tasks 259-262 в kip8)»: CACHE_VERSION = kipia-v404,
  v403 убрана, тестовые v517-v519 в боевом sw.js отсутствуют.
- tests/test-prod-calendar.js: НОВЫЙ набор (1046 строк, Tasks
  260+262: 75 тестов — реальные данные legalic 2025-2027, официальные
  нормы, День шахтёра, цепочки переносов, приоритеты источников, кэш
  v2, TTL/офлайн) перенесён; единственная SW-ссылка адаптирована
  kipia-test-v519 → kipia-v404. tests/run-all.js: регистрация набора.
- Тесты: 910 → 1003 passed / 0 failed (node tests/run-all.js).
  Разница с kip8test (1009) = 6 — убранные тестовые SW-секции,
  норма для порта (в Task 258-порте было так же).
- Верификация браузерная на РЕАЛЬНОМ kip8/index.html (http.server
  127.0.0.1:8931, agent-browser/Chromium): 0 JS-ошибок;
  ProdCalendar.ensureYear(2026) — legalic загружен ЖИВЫМ запросом
  (кэш ws_pcal_year2_2026_42, source legalic, версия
  RU-FEDERAL-2026-v1 — БЕЗ префикса kip8test:, как и должно быть в
  боевом репо); dayInfo(2026,8,30) — «День шахтёра», regional,
  off+holiday; dayInfo(2026,1,9) — «Выходной, перенесённый с
  03.01.2026», transferFrom; шторка: «Норма на 2026 год (40-час.):
  1972 ч» + бейдж «официальные нормы» + строка «30 августа —
  региональный праздник — День шахтёра»; 2027: бейдж
  «предварительные данные». Скриншоты:
  scripts/task262-port-kip8-{sheet,2027,grid}.png. (Сетка в
  песочнице ждёт сотрудников — Apps Script бэкенд недоступен из
  контейнера; код идентичен kip8test, где Task 262 верифицирован
  харнессом 34/34 + VLM.)
- Промт обновлён: v404, 1003 тестов, post-Task 262 (запись о
  переносе партии + таблица версий kip8 v404 / kip8test v519).
- Коммит + push origin/main (PAT-протокол, URL сброшен, утечек нет).

Stage Summary:
- Task 262 (порт) выполнен и запушен в боевой kip8 (kipia-v404,
  1003 passed): в «Графике работы» теперь вся партия Tasks 259-262 —
  бордюрчик с эффектом выступа, сотрудники сгруппированы (сменные
  1-5 → дневные по алфавиту, разделитель групп), производственный
  календарь РФ автоматически загружается с calendar.legalic.ru
  (официальные нормы 40/36/24 ч, цепочки переносов в тултипах,
  версии календаря, 2027 «предварительные данные»), резерв isDayOff,
  опция production-calendar.ru с токеном; День шахтёра Кузбасса
  (последнее воскресенье августа, Закон КО № 186-ОЗ) вычисляется
  автоматически для региона 42 на любых данных и в офлайн-фолбэке.
- Боевой сайт https://bloknett-design.github.io/kip8/ — пользователям
  нужно обновление PWA (кэш v404).
- Серверная часть (Apps Script) не затронута — только фронтенд;
  scripts/WorkSchedule.gs идентичен (в Tasks 259-262 не менялся).
- Локальная дата: 2026-08-30 (Asia/Novosibirsk, UTC+07:00).

---
Task ID: 268
Agent: main (Super Z)
Task: Перенос последних изменений (партии Tasks 264-267) из
      kip8test@76d84e5 в боевой kip8 — по указанию пользователя
      «Перенеси последние обновления в боевой kip8».

Work Log:
- Базовая линия: kip8 @ cf3a0fc (kipia-v404, чисто; режимы файлов
  нормализованы core.fileMode=false), remote синхронен. Неперенесённое:
  Tasks 264/265/266/267 (0f687e5, 8153254, 091e7bf, 76d84e5).
- index.html: git-патч aad96f4..76d84e5 (962 строки diff) применился
  к kip8 ЧИСТО (git apply --check + apply); скан на
  kip8test/kipia-test/bloknett в добавленных строках — 0 вхождений
  (репо-специфика не затронута). 37 831 → 38 069 строк.
- sw.js: kipia-v404 → v405 (партия = 1 инкремент) + комментарии
  Tasks 264-267 в шапке.
- Тесты (все замены со счётчиками):
  test-prod-calendar.js — шапка (пункт 8), CSS-тест окошка, SW-ссылка
  → kipia-v405, Task 262-секция адаптирована (бейдж «официальные
  нормы» удалён, День шахтёра «Закон», окошко/тултип), + describe
  Task 264 (День города ×6, окошко ×5) и Task 266 ×8 из kip8test
  дословно (версий не содержат); файл стал побайтово равен kip8test
  (1223 строки) кроме SW-строки v405. test-work-schedule.js — тесты
  Task 249 переписаны под полную цепочку крошек (dashboard → docs-ios,
  1→3 и 2→4 сегмента), SW-секция «Task 262: SW v404» → проверка
  актуальной v405 + защита от тестовых версий расширена v517-v523,
  + describe Task 265 ×10 (vm-песочница kipConfirm) и Task 267 ×17
  (точки ×4, docs-ios ×4, закрепление ×4, крошки ×3, SW v405 ×2 —
  SW-секция адаптирована kipia-test-v523 → kipia-v405, старая v522 →
  v404). Итог: 1049 passed / 0 failed (было 1003; +46 = дельта
  kip8test 1009 → 1055, норма порта).
- Верификация браузерная на РЕАЛЬНОМ kip8/index.html
  (scripts/port-264-267-browser-check.py, http.server + Playwright,
  1280×800): 29/29 PASS — живой legalic (кэш ws_pcal_year2_* БЕЗ
  тест-префикса); окошко wsCalPanel: 2 столбика (нормы слева,
  праздники справа), окошко в левой части бара / кнопки в правой,
  статическая высота 160px, overflow-y auto, нет «источник:»/
  «обновлено»/«официальные нормы», нормы 40/36/24; июнь 2026 —
  «День России · День города Кемерово» + 1972 ч/год; август 2026 —
  День шахтёра; 2027 — «предварительно»; диалог: клик «Сформировать»
  → «Формирование шахматки» («Сформировать»/«Отмена»), Escape
  закрывает без генерации (песочница: WorkSchedule._canEdit=true);
  docs-ios: один .menu-btn-row, кнопки в один ряд, 3 колонки, равная
  высота, обёртка data-subsection-key=work-schedule; свайп →
  pinnedSubsections + ярлык «График работы» на главной; крошки
  «Главная / Документация / Документация ИОС / График работы»
  (кликабельные dashboard/docs/docs-ios); wsEmpty — 3 спана
  flowLoadingDot 1.4s с задержками 0s/0.2s/0.4s; 0 JS-ошибок.
  VLM (glm-5v-turbo) ×4 — окошко (2 столбика, кнопки справа), диалог
  открытый (заголовок/кнопки/затемнение), крошки (полная цепочка,
  текущий сегмент жирный). Скриншоты: scripts/port-264-267-*.png.
- Промт обновлён: post-Task 267 (перенос партии 264-267), кэш v405,
  1049 тестов, таблица версий (kip8 v405 / kip8test v523), детальный
  пункт партии в разделе переноса.
- Коммит + push origin/main (PAT-протокол, URL сброшен, утечек нет).

Stage Summary:
- Перенос выполнен: боевой kip8 (kipia-v405, 1049 passed) содержит
  всю партию Tasks 264-267 — окошко производственного календаря в
  баре кнопок графика (нормы 40/36/24 + праздники месяца,
  звёздочка сокращённых дней, День города Кемерово 12 июня),
  «Сформировать» только через диалог «Формирование шахматки»,
  окошко столбиками (нормы слева/праздники справа, окошко слева/
  кнопки справа, статический бар 160px + скролл, без строки
  источника и бейджа), бегущие точки «Загрузка…», авто-размещение
  кнопок Документации ИОС, закрепление «Графика работы» на главной,
  полная цепочка крошек.
- Боевой сайт https://bloknett-design.github.io/kip8/ — GitHub Pages
  задеплоит автоматически; пользователям нужно обновление PWA
  (кэш v405).
- Серверная часть (Apps Script) не затронута — только фронтенд.
- Локальная дата: 2026-08-30 (Asia/Novosibirsk, UTC+07:00).
...
Task ID: 271
Agent: main (Super Z)
Task: Перенос последних обновлений (партии Tasks 269-270) из
      kip8test@c924c7a в боевой kip8 — по указанию пользователя
      «Перенеси все последние изменения в боевой kip8».

Work Log:
- Разведка: kip8test @ c924c7a (v525, 1078 тестов, чисто, синхронен
  с remote); kip8 локально @ 883c660 (v405, 1049 тестов, чисто,
  синхронен). Неперенесённое: Tasks 269 (184895f) и 270 (c924c7a).
- index.html: git-патч 53679a9..c924c7a (240 строк: +166/−74)
  применился ЧИСТО; скан добавленных строк на kip8test/kipia-test/
  bloknett — 0 вхождений. 38 069 → 38 161 строк.
- sw.js: kipia-v405 → v406 + комментарии Tasks 269/270 (партия =
  один инкремент).
- Тесты: test-prod-calendar.js — SW-тест Task 260 → v406, describe
  Task 266 переписан под новую структуру (ws-cp-group, 95px, кнопки
  слева внизу / окно справа); test-work-schedule.js — SW-секция
  Task 262 → актуальная v406 + защита от тестовых версий расширена
  до v517-v525, #wsEmpty-тесты Task 267 → flow-loading-разметка,
  describe «Task 267: SW v405» → «Task 269-270: SW версия v406»,
  + describe Task 269 ×4 и Task 270 ×2 из kip8test дословно (без
  SW-строк). Прогон: 1072 passed / 0 failed (1049 + 23 = дельта
  kip8test 1055 → 1078).
- Верификация: scripts/port-269-270-browser-check.py — 45/45 PASS на
  реальном kip8/index.html (живой legalic: группа норм .ws-cp-group
  с ДВУМЯ подстолбиками — дни слева, часы 40/36/24 справа; окно по
  ширине текста в правой части бара, кнопки 34px слева внизу; окно
  95px, бар 106px, отступы/зазоры 5px; плашки «Норма» синяя
  rgba(74,143,199,0.22)/#93c1ea, «Праздники» красная
  rgba(255,107,107,0.2)/#ff9c9c, обнимают текст; июнь 2026 «День
  России · День города Кемерово» + 1972 ч; август — День шахтёра;
  бар статичен июнь=июль=август=106px; «Загрузка…» .flow-loading-text
  16px/600; светлая: окно #e9e7de темнее тулбара #f0eee6, плашки
  #1d5f96/#b02c2c; мобильная 375px — колонка, 95px, 34px; кэш
  ws_pcal_year2_* без тест-префикса; 0 JS-ошибок) + доп. чек января
  2027 (каникулы 1-8 янв + Рождество 7 янв, бейдж «предварительно»,
  скролл, бар 106px) + VLM ×5. Скриншоты: scripts/port-269-270-*.png.
- Промт: task271-update-kip8-prompt.py (post-Task 270 в шапке, кэш
  v406, таблица версий kip8 v406 / kip8test v525, детальный пункт
  партии, счётчик 1072).
- Коммит + push origin/main (PAT-протокол, URL сброшен, утечек нет).

Stage Summary:
- Перенос выполнен: боевой kip8 @ kipia-v406 (1072 passed) содержит
  всю партию Tasks 269-270 — окно календаря в баре «Графика работы»:
  нормы ДВУМЯ подстолбиками (дни слева, часы справа), окно шириной
  по тексту в ПРАВОЙ части бара, кнопки единой высоты 34px в ЛЕВОЙ
  НИЖНЕЙ части, «Загрузка…» шрифтом расходомеров (16px/600); окно
  под 5 строк (95px), светлая тема — фон окна темнее (#e9e7de),
  заголовки столбцов — цветные плашки (синяя/красная) с ярким
  текстом, бар узкий 106px с отступами 5px.
- Боевой сайт https://bloknett-design.github.io/kip8/ — GitHub Pages
  задеплоит автоматически; пользователям нужно обновление PWA
  (кэш v406). Серверная часть не затронута.
- Локальная дата: 2026-08-31 (Asia/Novosibirsk, UTC+07:00).

---
Task ID: 273 (перенос)
Agent: main (Super Z)
Task: Перенос Task 272 из kip8test@7d69528 в боевой kip8 — по
      указанию пользователя «перенеси эту партию в боевой kip8»
      (партия = Task 272: кнопка «Обновить» с диалогом источника,
      шторка настроек удалена, регион фиксирован 42, «Сформировать»
      на весь год, кнопки в левом верхнем углу бара).

Work Log:
- Разведка: kip8test @ 7d69528 (v526, 1090 тестов, Tasks 269-272;
  269-270 уже в бою с Task 271), kip8 @ 5467dba (v406, 1072);
  PAT восстановлен в /home/z/.kip_pat (600), оба клона обновлены.
- index.html: git-патч c924c7a..7d69528 (786 строк: +238/−548)
  применился ЧИСТО (JS-ханки со смещением −14 — блока
  isolateLocalStorage в kip8 нет); 37 837 → 37 851 строк; скан
  добавленных строк на kip8test/kipia-test/bloknett — 0 вхождений.
  Ложная тревога «monthsNomi]» опровергнута od -c: байты
  monthsNom[mi] корректны (артефакт отображения grep/sed).
- sw.js: kipia-v406 → v407 + комментарий Task 272.
- Тесты: test-prod-calendar.js и test-work-schedule.js — патч
  Task 272 c отбоем SW-хунков (--reject), ручная адаптация
  SW-строк под нумерацию kip8 (v526 → v407; «Task 272: SW версия
  v407»; защита от тестовых версий расширена v517-v526; секция
  Task 269-270 проверяет v407). Прогон: 1084 passed / 0 failed
  (1072 + 12 = дельта kip8test 1078 → 1090).
- Верификация: scripts/port-272-browser-check.py (в my-project/
  scripts) — 36/36 PASS на РЕАЛЬНОМ kip8/index.html: чип «Обновить»
  (polyline-иконка, onclick confirmRefresh, тултип с источником);
  шторка/селект региона/поле токена ОТСУТСТВУЮТ, openSheet/saveToken
  undefined, getSettings() → {region:42}; кнопки в ЛЕВОМ ВЕРХНЕМ
  углу (top=бар+5), окно справа, бар 106px/окно 95px; июнь 2026
  «День России · День города Кемерово» + нормы 40/36/24; диалог
  «Обновление календаря» (источник legalic, дата, вопрос, кнопки,
  Escape); force-обновление (чип disabled «Обновление…»,
  force-запрос legalic, тост «Календарь обновлён»); «Сформировать» —
  12 последовательных вызовов 1..12/2026, прогресс N/12, тост
  «Год 2026: сформировано 120 зап., обновлено 24», loadGrid; сбой
  5-го месяца — вызовы 1..5, тост «4/12», сетка перегружена;
  мобильная 375px — колонка без переполнений; 0 JS-ошибок.
  Нюансы теста: KipToast в index.html не определён (82 ссылки с
  typeof-защитой) — внедрена заглушка; _kipDialogClose очищает
  innerHTML через 250мс — пауза перед повторным открытием диалога.
  VLM ×2 — ок (кнопка/окно/без региона и токена; диалог). 
  Скриншоты: scripts/port-272-{toolbar,june,dialog,year,mobile}.png.
- Промт: scripts/port-272-update-prompt.py (в my-project/scripts) —
  post-Task 272 в шапке, кэш v407, таблица версий kip8 v407 /
  kip8test v526, актуализированы описания ProdCalendar (шторки
  нет, регион фиксирован), детальный раздел Task 272 в истории,
  счётчик 1084.
- Коммит + push origin/main (PAT-протокол, URL сброшен, утечек
  нет).

Stage Summary:
- Перенос выполнен: боевой kip8 @ kipia-v407 (1084 passed) содержит
  Task 272 — кнопка «Календарь» → «Обновить» (диалог «Обновление
  календаря» с источником calendar.legalic.ru и подтверждением),
  шторка настроек удалена (регионы и токен production-calendar.ru
  убраны — регион фиксирован 42 — Кузбасс, цепочка legalic →
  isDayOff), «Сформировать» строит шахматку на ВЕСЬ ГОД (12 месяцев,
  прогресс N/12, суммы в тосте, остановка при сбое месяца), кнопки
  — в ЛЕВОМ ВЕРХНЕМ углу бара (окно данных справа, бар 106px).
- kip8test (7d69528, v526) и kip8 синхронизированы. Боевой сайт
  https://bloknett-design.github.io/kip8/ — GitHub Pages задеплоит
  автоматически; пользователям нужно обновление PWA (кэш v407).
  Серверная часть не затронута (generateMonth на сервере прежний).
- Локальная дата: 2026-08-31 (Asia/Novosibirsk, UTC+07:00).
...
Task ID: 285 (перенос)
Agent: main (Super Z)
Task: Перенос партии Tasks 274-284 (kip8test@bb44f97 — модуль отпусков)
      в боевой kip8 — по указанию пользователя «заработало. теперь
      перенеси все последние изменения в боевой kip8».

Work Log:
- Контекст: пользователь подтвердил работу отпусков в kip8test после
  Task 284 (переключение на развёртывание AKfycbyt…/exec). kip8@531b49a
  (v407) синхронизирован с origin; PAT от 24.08.2026 в /home/z/.kip_pat.
  Неперенесённое: Tasks 274-284 (12 коммитов, +5078/−73, 23 файла).
- КЛЮЧЕВОЕ ОТКРЫТИЕ: WEB_APP_URL в kip8 — AKfycbyt…/exec С TASK 245,
  т.е. боевой фронтенд УЖЕ ходил в то же развёртывание, которое
  пользователь обновил при отладке (Task 283) и на которое переключён
  kip8test (Task 284). ОДИН бэкенд на оба фронтенда → Apps Script
  менять не нужно, серверная часть уже работает (подтверждено
  «заработало»).
- index.html: git-патч 7d69528..bb44f97 (1190 строк, 20 ханков) —
  URL-ханк Task 284 отброшен фильтром (в kip8 URL уже целевой),
  остальные 19 ханков легли ЧИСТО (+883/−18, смещения −14/−19 —
  isolateLocalStorage в kip8 нет); добавлен kip8-адаптированный
  комментарий к WEB_APP_URL (Task 284/285). Сверка вхождений с
  эталоном: work-schedule-vacations 10=10, listVacations 3=3,
  _apiErrText 4=4, kip-dialog-alt 6=6, serverMessage 6=6 — разница
  19 строк = блок изоляции kip8test (14) + комментарий Task 284 (5).
  0 тест-маркеров в добавленных строках.
- sw.js: kipia-v407 → v408, комментарий партии Tasks 274-284.
- Серверные справочные файлы: WorkSchedule.gs скопирован целиком
  (база 7d69528 была идентична kip8), Code.gs — точечная правка
  (3 case отпусков + комментарий URL; 3 док-строки kip8 сохранены),
  VacationsInit.gs / VacationsDiagnose.gs / task283-probe-live.sh —
  новые копии; DEPLOY-доки Tasks 274/279/281/282/283/284 — копии.
  SPREADSHEET_ID во всех файлах один (1MQtW-…, табель_КИП_ИОС).
- Тесты: 5 новых файлов (test-vacations-generate/init/feedback/
  diagnose + test-deploy-url) + обновления test-work-schedule.js /
  test-prod-calendar.js целиком из kip8test с адаптацией SW-ассертов
  (kipia-test-v531 → kipia-v408, негативные v514/v517/v523/v525/v527
  → v407/v406/v404/v405/v407; описания «Task 284» → «Task 285 —
  перенос в kip8», v531→v408 в текстах); run-all.js — дубль
  require(test-prod-calendar) устранён, 5 require добавлены.
  Прогон: 1230 passed / 0 failed (1084 + 146 = счётчик kip8test@bb44f97).
- Верификация: scripts/task285-browser-check.py (адаптация task282,
  cwd → kip8) — 21/21 PASS на РЕАЛЬНОМ kip8/index.html: KipToast
  определён приложением (typeof object, реальный div#toast); страница
  «Отпуска» строится (карточки, части периодов); addVacation отклонён —
  тост с serverMessage «Период пересекается… (2026-06-01 — 2026-06-14)»,
  диалог остаётся открыт; валидация «Выберите сотрудника» в тосте;
  успех — диалог закрыт, тост «Отпуск добавлен…», токен приложён,
  payload ISO; 0 JS-ошибок. VLM ×2 (тост успеха: субнавигация
  Шахматка/Сотрудники/Инструктажи/Отпуска + карточки; тост ошибки:
  диалог «Новый отпуск» открыт, поля заполнены, текст читабелен).
  Скриншоты: scripts/task285-{toast-error,toast-success}.png.
- Доки: DEPLOY-Task285-vacations-module.md (1 шаг: Ctrl+Shift+R ×2;
  Apps Script менять НЕ нужно — развёртывание то же с Task 245);
  промт: шапка post-Task 285, кэш v408, таблица (kip8test v531 /
  kip8 v408), секция истории «Перенос партии Tasks 274-284», чеклист
  1230 passed.
- Коммит + push (PAT-протокол, URL сброшен, утечек нет).

Stage Summary:
- Перенос выполнен: боевой kip8 содержит модуль отпусков (Tasks
  274-284): лист «Отпуска» табель_КИП_ИОС, серверный CRUD + шаг 4.5
  generateMonth («О», приоритет «руч > отпуск > инструктаж > смена»),
  страница «Отпуска» + субнавигация, VacationsInit/VacationsDiagnose,
  диалог «Сформировать» год/месяц, видимые тосты ошибок сервера.
  SW kipia-v408, тесты 1230/0, browser-check 21/21, VLM ×2.
- Бэкенд общий с kip8test (AKfycbyt…, с Task 245): серверная часть
...
Task ID: 306-перенос
Agent: main (Super Z)
Task: Перенос партии Tasks 298-306 из kip8test@4fc48bc в боевой kip8
      (по указанию пользователя «Перенеси все последние изменения в
      боевой kip8») — одним инкрементом SW kipia-v410→v411.

Work Log:
- Клон kip8 (PAT-протокол, URL сброшен после); HEAD = 70f8e40:
  index.html/sw.js/tests/scripts с 623ffd0 не менялись (docs 31605ce
  + авто-коммиты data) — базовая линия синхронна kip8test@3bf42a4.
- index.html: патч 3bf42a4..4fc48bc (38 хунков, 1006 строк) применён
  git apply — все хунки с offset −15, конфликтов нет. Верификация:
  diff(kip8test@4fc48bc, kip8-новый) == базовый изоляционный дифф
  Task 292 по содержанию (82 строки; только сдвиг номеров строк
  на +473) — байт-в-байт, изоляция не затронута.
- sw.js: kipia-v410 → v411 + комментарий партии Tasks 298-306 в
  шапке (методика «одна партия = один инкремент»; тестовые v540-v545
  в kip8 не существовали).
- tests/: 12 файлов из дельты — 4 новых (test-work-events.js,
  test-tab-numbers.js, test-vacation-shift.js, test-task306.js) +
  8 обновлённых; SW-ассерты kipia-test-v545 → kipia-v411,
  негативные ассерты старых версий → kipia-v410; run-all.js +4
  require; заголовок теста SW в test-deploy-url.js актуализирован
  (v411, Task 306-перенос).
- scripts/: WorkSchedule.gs скопирован байт-в-байт (базовая линия
  3bf42a4/623ffd0 совпадала — проверено diff); НОВЫЕ справочные
  копии StatusCodesInit.gs (Task 299) и TabNumbersFix.gs (Task 304).
- Сьют: node tests/run-all.js = 1433 passed / 0 failed — ПОЛНЫЙ
  ПАРИТЕТ с kip8test@4fc48bc (1324 → 1433).
- Browser-check: my-project/scripts/task306-browser-check.py (порт 8931,
  python http.server, мок fetch по action, скриншоты /tmp) — 29/29
  PASS: бейдж плановой смены только у «сменного» 017 (6 бейджей
  Д/Н); дневной 023 — план «ОТ» без бейджей; бейдж «ПР» #EF5350 на
  смене «Н»; пунктирный «*» на пустой ячейке; тултипы; форма 5
  типов; тулбар БЕЗ «Обновить», «Сформировать» одна; клик → диалог →
  месяц = 1 вызов generateMonth + ТИХОЕ обновление календаря
  (spy refreshNow(true)); год = 12 вызовов; окошко норм живо;
  0 JS-ошибок; localStorage БЕЗ префикса kip8test:.
- Промт: my-project/scripts/task306-transfer-update-prompt.py — строка 3
  post-Task 306 (перенос партии 298-306, полный состав), строка 4 =
  post-Task 298 + цепочка 296…; кэш v411; счётчики 1324→1433 /
  20→24 файла / ~39→~39.8 тыс. строк / WEB_APP_URL ~28 176; секция
  модуля «График работы»: 4 страницы, 14 endpoints, актуальное
  состояние; НОВАЯ секция партии 298-306 (после Task 285);
  примечания Task 306 в «Интеграции в шахматку» и секции Task 272;
  «Последняя принятая партия» — Tasks 298-306 / kipia-v411.
- DEPLOY-Task306-transfer.md: Apps Script WorkSchedule.gs + «Новая
  версия» (если ещё не задеплоен из kip8test — ОДИН бэкенд);
  листы НЕ трогать (16 кодов уже в живом листе); фронтенд —
  Ctrl+Shift+R ×1–2; откат; что НЕ делать.
- node --check sw.js OK; коммит + push (PAT-протокол, URL сброшен).

Stage Summary:
- Партия Tasks 298-306 (коды Т-12/Т-13, мероприятия И/ОБ/ПЗ/ПР/*,
  таб_№ текстом, отпуск + плановая смена бейджем только у «сменных»,
  одна кнопка «Сформировать» с тихим обновлением календаря)
  перенесена в боевой kip8: SW kipia-v411, тесты 1433/0 (паритет),
  browser-check 29/29, изоляционный паритет байт-в-байт.
  ДЕПЛОЙ ПОЛЬЗОВАТЕЛЯ: (1) если Apps Script WorkSchedule.gs ещё не
  обновлялся после Tasks 298/303/304/306 — заменить файл + «Новая
  версия» у AKfycbyt…; (2) Ctrl+Shift+R ×1–2 на
...
## Task 310-перенос — рамка д/н 1px с закруглением, «Старт цикла», отпуск × праздники × лимит 42 (из kip8test@5fe5ab1)

**Дата:** 2026-09-04 · **SW:** kipia-v414→v415 (один инкремент) · **Сервер:** не менялся

**Заявка:** «у ячеек с строчными "д"/"н" переделай рамку 2px на 1px и с
закруглёнными углами. В карточке сотрудника убери строку "Старт цикла",
итог подсчёта количества дней отпуска должен суммироваться с учётом
вычета праздничных нерабочих дней попавших на период отпуска (максимальное
количество отпуска в год не должно превышать 42 дня на химическом
производстве по ТК РФ)».

**Что сделано (см. scripts/DEPLOY-Task310-vac-holidays-limit.md):**
- Рамка ручных д/н: box-shadow inset 2px → ::before border 1px +
  border-radius 3px (как свотч .ws-popup-swatch; border-collapse
  игнорирует radius на td; pointer-events:none; ws-source-manual под ней).
- «Старт цикла» убран из карточки _renderEmpPopup (поле живо в шторке
  «+ Сотрудник» и на сервере).
- Праздники ст. 112 не расходуют отпуск (ст. 120 ТК РФ): хелперы
  _vacIsHoliday/_vacSplitDays/_vacNetDays/_vacNetDaysInYear +
  _VAC_YEAR_LIMIT=42 (28+7+7). Карточка: «N дней (−K праздн.)», итог
  «… · лимит 42» / красный «ПРЕВЫШЕН» (ws-emp-overlimit). Тултип плана
  «ОТ» — чистые дни. Шторка: «в счёт отпуска M (вычтено праздников: K,
  ст. 120 ТК РФ)» + НОВАЯ строка #wsVacYearInfo «Год YYYY:
  запланировано X из 42 …; с этим периодом: Y» (ws-vac-overlimit);
  submitVacationForm — блокировка >42 ДО _api addVacation.
- Перенос: патч index.html 379 строк (git apply, чисто); sw.js v415;
  tests/: test-task310.js НОВЫЙ (+22; vm-тесты хелперов с моком
  ProdCalendar), test-task309.js/test-work-schedule.js копии с
  адаптацией версий (v549→v415/v548→v414 и исторические) и
  актуализацией (рамка ::before; «Старт цикла»; окно submitVacationForm
  2600→4600), 10 файлов бамп v414→v415, run-all.js +require.
  Сьют: 1476 → 1498/0 (ПАРИТЕТ с kip8test@5fe5ab1).
- Верификация: task310-browser-check.py (порт 8934) — 20/20 на
  РЕАЛЬНОМ kip8/index.html (рамка 1px/3px ×4 угла; «Старт цикла» нет;
  Иванов 01–14.06 = 13 дн. (−1 праздн. 12.06); Петров 43 из 42 —
  красный ПРЕВЫШЕН; шторка 13→27 проходит, 43+14=57 — блок, тост,
  addVacation НЕ вызван; 375px; «ИТР8 pro» — кнопка скрыта; 0
  JS-ошибок); регресс: task309 27/27, task307 22/22, task308 21/21
  (порт 8928 из каталога kip8); VLM ×3 (рамка/карточка/шторка).
- Промт: версия post-Task 310, запись «Перенос Task 310», партия
  298-310, счётчики 1498/28 файлов/v415.
- Деплой: GitHub Pages сам; пользователю Ctrl+Shift+R ×1–2 (SW v415);
  Apps Script/листы НЕ трогать; kip8-desktop — автосинк.

---
Task ID: 311-перенос
Agent: main (Super Z)
Task: Перенос Task 311 из kip8test@a949052 в боевой kip8 (PAT от 24.08.2026):
      карточка сотрудника зачищена («Шаблон ротации», «Сотрудник», «Итого в
      году» убраны), пояснительные окна при наведении удалены (карточка —
      только клик; title-тултипы с ячеек/шапки сняты), кнопка «+ Сотрудник»
      заменена заголовком «Сотрудник» в шапке сетки.

Work Log:
- База: kip8@1e488d8 (v415, 1498/0). Патч index.html 765 строк
  (diff 5fe5ab1..a949052, git apply — чисто).
- sw.js: kipia-v415→v416 + комментарий Task 311.
- tests/: test-task311.js НОВЫЙ (kipia-test-v550→kipia-v416, +13);
  копии с адаптацией версий (v550→v416, v549→v415) и актуализацией
  под Task 311: test-task309/310, test-work-schedule, test-prod-calendar,
  test-work-events, test-vacation-shift, test-task306,
  test-vacations-generate, test-deploy-url, test-tab-numbers,
  test-flow-period-input, test-vacations-feedback; run-all.js +require;
  test-task309: старая версия v547→v414 в негативном ассерте.
- Сьют: 1498 → 1511/0 (ПАРИТЕТ с kip8test@a949052); 4 script-блока —
  синтаксис чист.
- scripts/: НОВЫЕ task311-browser-check.py (порт 8936) — 24/24 на
  РЕАЛЬНОМ kip8/index.html (кнопки нет; заголовок-кнопка открывает шторку;
  наведение не открывает ничего; 0 title в сетке; карточка по клику без
  трёх строк; чистые дни живы; зритель без функции добавления; 0
  JS-ошибок); task309/310/307/308-browser-check актуализированы под
  Task 311 (порты 8934/8928) — 26/26, 20/20, 20/20, 21/21;
  DEPLOY-Task311-card-clean-nohover-headadd.md; пруфы PNG.
- Промт: версия post-Task 311 (заголовок + буллет переноса + «Последняя
  принятая партия» Tasks 298-311, счётчики 1511/v416).

Stage Summary:
- Task 311 в бою: kip8@<этот коммит>, kipia-v416, тесты 1511/0 (паритет),
  browser 24/24 + регресс 87/87 (309/310/307/308). Что получил
  пользователь: карточка сотрудника лаконичнее (без шаблона ротации,
  заголовка и итога года — лимит 42 контролирует шторка «+ Отпуск»);
  никаких всплывающих окон при наведении (карточка — строго по клику);
  добавление сотрудника — клик по заголовку «Сотрудник +» в шапке
  (только редакторам). Пользователю: Ctrl+Shift+R ×1–2; Apps Script и
  листы НЕ трогать; kip8-desktop — автосинк. Следующий номер: 312.

---
Task ID: 312-314-перенос
Agent: main (Super Z)
Task: Перенос ПАРТИИ Tasks 312+313+314 из kip8test@894afab в боевой kip8 (PAT от 24.08.2026) одним инкрементом SW kipia-v416→v417 (по заявке пользователя «перенеси в kip8 и kip8-desktop»):
      Task 312 — «+ Отпуск» в карточку сотрудника + чистый попап ячейки;
      Task 313 — подсветка сегодняшней даты + окно «Мероприятия в этот день»;
      Task 314 — «·» с фоном пустой ячейки + бейджи «Инструктажи» + локальная
      копия графика + кнопка «Обновить» данных.

Work Log:
- База: kip8@30be9a0 (v416, 1511/0). Патч index.html 1388 строк
  (diff kip8test a949052..894afab, git apply — чисто; верификация:
  diff(kip8test@894afab, kip8-новый) == базовый изоляционный дифф
  55 строк — префиксы kip8test_/isolateLocalStorage/комментарии
  Task 243/242/284, зоны партии не затронуты); 4 script-блока
  node --check OK.
- sw.js: kipia-v416→v417 + комментарий партии 312+313+314
  (один инкремент, только фронтенд).
- tests/: НОВЫЕ test-task312.js (+17), test-task313.js (+18),
  test-task314.js (+38) — адаптация версий (v550/v551/v552→v416,
  v553→v417; комментарии о инкрементах переписаны под kip8);
  копии с адаптацией версий 14 файлов: test-deploy-url,
  test-flow-period-input, test-prod-calendar, test-tab-numbers,
  test-task306, test-task309, test-task310, test-task311,
  test-vacation-shift, test-vacations-feedback,
  test-vacations-generate, test-work-events (бит-в-бит на точке
  синка), test-work-schedule, run-all.js (+3 require);
  негативные ассерты kipia-test-v514…v546 — намеренно оставлены
  (проверка отсутствия тестовых версий в проде, как в kip8 и было).
- Сьют: 1511 → 1584/0 (ПАРИТЕТ с kip8test@894afab), 1312 мс.
- scripts/: НОВЫЕ task312-browser-check.py (порт 8938), task313-
  browser-check.py (порт 8939), task314-browser-check.py (порт
  8940) — копии kip8test с адаптацией PORT и токенов
  (-k8); регресс task307/308/309/310/311-browser-check.py —
  копии kip8test@894afab (актуализированы под партию) с адаптацией
  PORT (8928/8928/8934/8934/8936) и токенов -k8; пруфы PNG ×13
  новых + ×16 регресс перегенерированы на РЕАЛЬНОМ kip8/index.html.
- DEPLOY-доки: DEPLOY-Task312-vac-in-card-popup-clean.md,
  DEPLOY-Task313-today-highlight-events-window.md,
  DEPLOY-Task314-dot-badges-cache.md — копии с адаптацией
  (SW-строки → v416→v417, порты 8938/8939/8940, GitHub Pages kip8).
- Промт: task312-314-transfer-update-prompt.py + прогон (версия
  post-Task 314 «ПЕРЕНОС ПАРТИИ», кэш v417 в шапке/таблице/примере,
  kip8test-строка v539→v553, 40.5 тыс. строк, 1584 теста/32 файла,
  партия 298-314, буллет переноса в журнале переносов).
- Верификация браузером (реальный kip8/index.html, мок fetch):
  task312 31/31 (кнопка из тулбара удалена, строка «+ Отпуск…»
  в карточке с префиллом; попап: «— выходной —» нет, коды
  мероприятий не в списке основных, очистка в «Дополнительно…»);
  task313 30/30 (колонка «сегодня» одна, шапка-градиент+акцент,
  ячейки ws-today поверх inline-цветов, ручная — две inset-тени;
  окно «Мероприятия в этот день» НАД окном кодов: зазор 4–12px,
  левые края ±1px, z-9402; секции в окне кодов нет; пустой день;
  Esc закрывает оба; 375px; зритель — подсветка видна, окон нет);
  task314 36/36 (кэш-открытие 0 запросов workSchedule.*; кнопка
  «Обновить»: спиннер/блокировка/тост/штамп/запись кэша; «·»/
  ws-dot-code/БЕЗ inline-фона/фон = фон пустой рабочей ячейки;
  И-день: «·»+сплошной бейдж #B3E5FC; ОТ+бейдж; пустая+пунктирный;
  Д+бейдж; Б без бейджей; попап «·»/ws-swatch-dot; select «·»;
  светлая #eef0f2; смена месяца — сеть/возврат мгновенно; офлайн —
  сетка не стёрта; зритель: «Обновить» доступна, «Сформировать»
  скрыта; мобильный 375px; 0 JS-ошибок);
  регресс: task307 20/20, task308 20/20, task309 26/26,
  task310 20/20, task311 24/24; VLM ×4 скриншота
  (events-window/badges/light-dot/emp-card) — без дефектов.
- Сервер (WorkSchedule.gs, scripts/*.gs), листы, Apps Script —
  НЕ менялись; charts-desktop.js / devices-table-desktop.js /
  manifest.json — не менялись.

Stage Summary:
- Партия 312+313+314 в бою: kip8@<этот коммит>, kipia-v417,
  тесты 1584/0 (паритет), browser 31+30+36 + регресс 110
  (307-311). Что получил пользователь: кнопка «+ Отпуск» —
  строкой в блоке отпусков карточки сотрудника; попап ячейки —
  только основные коды (без «— выходной —» и мероприятий);
  сегодняшняя дата подсвечена в шахматке; клик по ячейке — окно
  «Мероприятия в этот день» НАД окном кодов; «.» показывается
  как «·» на фоне ПУСТОЙ ячейки; мероприятия — маленькие бейджи
  во всех ячейках; график открывается МГНОВЕННО из локальной
  копии (0 сетевых запросов), данные обновляются кнопкой
  «Обновить» (не «Сформировать»; спиннер, тост, штамп «данные
  от …», сбой не стирает сетку, офлайн-толерантность).
  Пользователю: Ctrl+Shift+R ×1–2 (SW v417) + заменить цвет «.»
  в листе «Коды_статусов» на #EEF0F2. Apps Script/листы НЕ
  трогать. kip8-desktop — автосинк sync-to-desktop.yml (проверен
  отдельно). Следующий номер: 315.

---

## Task 315-330 — ПЕРЕНОС ПАРТИИ из kip8test@f9e1a3c в kip8 (один инкремент SW kipia-v417→v418)

**Дата:** 2026-09-06
**Заявка:** «…перенеси все последние изменения в боевой kip8 и kip8-desktop».

**Состав:** 16 задач (315-330) — бар из трёх равных частей с окнами
мероприятий/норм и 3 рядами кнопок; подсветка/фильтр дня столбца;
тултип «данные от»; увольнение (сервер dismissEmployee — Apps Script
уже развёрнут, бэкенд общий); попап/карточка/шапка/часы д/н/колонка
ФИО по тексту/зебра и линии шторки; BUGFIX Task 326 (вложенность DOM
— не открывались разделы и карточки расходомеров); «Итоги учёта»:
столбцы Явки/Часы/Переработка + доп. за ШЕВРОНОМ на левом краю
(бортик, ширина РОВНО по столбцам, переработка в ДНЯХ, зебра ФИО);
тумблер перекрёстной подсветки; шапка сине-серая + мелкий шрифт
описаний кодов + чёрный текст окон бара в светлой теме.

**Перенесено:**
- index.html — патч 3950 строк (git apply ЧИСТО, изоляция 0 строк,
  44 175 строк, node --check 4 блока OK);
- sw.js kipia-v417→v418 (один инкремент партии);
- СЕРВЕРНАЯ ЧАСТЬ: scripts/Code.gs (+5) и scripts/WorkSchedule.gs
  (+336) — синхронизированы файлы (Apps Script НЕ трогать — уже
  развёрнут с сессий kip8test, WEB_APP_URL общий);
- tests/ — все 48 файлов из kip8test с адаптацией версий (v569→v418,
  v570→v419, исторические v5XX→v417) + run-all.js;
- доки: browser-check партии (порты 8980-8997),
  DEPLOY-Task315-330-batch-transfer.md,
  task315-330-transfer-update-prompt.py (промт post-партия).

**Верификация:**
- тесты 1584 → **2010/0** (ПАРИТЕТ с kip8test@f9e1a3c);
- браузер на РЕАЛЬНОМ kip8/index.html (13 скриптов, 420 проверок):
  315=33/33, 316=28/28, 317=49/49, 318=45/45 (актуализирован под 330),
  319=48/48 (актуализирован), 320=24/24, 327=30/30, 328=36/36,
  329=43/43, 330=27/27 + регресс 312=31/31, 313=30/30, 314=36/36;
  321-325 — устаревшие до 329 (падают одинаково в обоих репо,
  superseded 327/329 — как в kip8test).

**Деплой:** GitHub Pages + Ctrl+Shift+R ×1–2 (SW v418); Apps Script/
листы НЕ трогать (сервер уже развёрнут); kip8-desktop — автосинк
sync-to-desktop.yml. Следующий номер задачи: 331.

---

## Task 331-333 — ПЕРЕНОС ПАРТИИ из kip8test@f965030 в kip8 (один инкремент SW kipia-v418→v419)

**Дата:** 2026-09-06
**Состав партии:**
- **Task 331** — шторка «Итоги учёта»: без вертикальных полосок
  (Firefox thin), кастомные горизонтальные ползунки ПОД бордюрами на
  одном уровне (зона 12px, без переполнения ПУСТАЯ), шеврон 11px/0.55,
  левый бордюрчик → полоса 2px #4a8fc7/#6e8ba4, шапка столбцов = шапке
  сетки (38px, var --ws-tt-head-h), «Явки (дни)»/«Переработка (дни)»,
  текст по центру, нижние бордюры колонок всегда на одном уровне
  (#wsGridCol: wrap + статичный #wsGridFoot + #wsGridHbar).
- **Task 332** — подсказка #wsCrossTip значка подсветки (окно формата
  «Обновить», живое состояние), КНОПКА ВИДА #wsViewBtn (самая левая
  ряда 3: полный/сменный/дневной по кругу, localStorage
  kip8_ws_view_v1, иконки 4 полосы 2 группами, подсказка #wsViewTip,
  гейты шторки, фильтр строк _viewEmployees), роль «КИП ИОС дежурный»
  — вид заперт на «сменный» (хук _onRoleUpdate), шторка «Год» без
  своей кнопки «Обновить» (обновление — кнопкой тулбара).
- **Task 333** — шапка шторки = шапке сетки (#1e293b/#bfcad5, ячейка
  «Сотрудник» в шапке), разделитель групп в шторке как в шахматке
  (ws-group-first 2px), перекрёстная подсветка → строки шторки
  (_rowClass + штампы в рендерах + inset-заливка), кнопка вида с
  ПОДПИСЬЮ «Вид», АРХИВ годовой таблицы ВОЗВРАЩЁН (блок «Архив» под
  основной таблицей, своя колонка «Сотрудник» на любом экране,
  мобайл-год получил имена активных), прижимка синхронизации скролла
  снята (архив ДОСТУПЕН прокруткой), полоса ФИО/ячейки через ::after
  (border-right в border-collapse уезжал — пробы Chromium+Firefox).

**Перенос:**
- патч index.html 2052 строк (diff kip8test f9e1a3c..f965030,
  git apply ЧИСТО; изоляционных строк в диффе 0);
- sw.js kipia-v418→v419 (один инкремент партии);
- tests/ — все 50 файлов из kip8test с адаптацией версий
  (v572→v419 текущая, v573→v420 сторож); тесты 2010→**2055/0**
  (ПАРИТЕТ с kip8test@f965030); node --check 4 блока OK;
- scripts/: task331/332/333-browser-check.py (актуализированы под
  333), task333-probe-separator.py, DEPLOY-Task331/332/333-*.md;
- промт: task331-333-transfer-update-prompt.py → post-Task 331-333
  (v419, 2055/50, ~45.0 тыс. строк, ~2.7 MB).

**Верификация на РЕАЛЬНОМ kip8/index.html (порт 8944/8997/8995):**
- task333 39/39 (десктоп/светлая 1024/мобайл 375/Firefox: подпись
  «Вид» 13px/600, цикл full→shift→day→full, шапка #1e293b = сетке,
  ws-group-first 2px rgb(74,143,199), подсветка строки шторки тем же
  индексом + inset-заливка, год: «Архив» блоком + архив достигается
  прокруткой (G9), полоса ФИО до/после прокрутки — пиксельные пробы
  Chromium+Firefox, 0 JS-ошибок);
- task332 44/44 (актуализирован: кнопка вида шире с подписью, полоса
  через ::after computed, год: «Сотрудник» в DOM — десктоп скрыт,
  архив блоком);
- task331 38/38 (актуализирован: шапка года скрыта — «Обновить»
  удалён; годовая таблица влезает — ползунок ПУСТ, зона 12px);
- регресс: task328 36/36 (порядок ряда: вид → перекрестье →
  Сформировать), task330 27/27.

**Серверная часть:** НЕТ (только клиент — Apps Script/листы не
трогать).

**Деплой:** GitHub Pages + Ctrl+Shift+R ×1–2 (SW kipia-v419);
kip8-desktop — автосинк. Следующий номер задачи: 334.

---
## Task 334 — ПЕРЕНОС из kip8test@06fbbae: «Вид» без значков + липкий блок карточки прибора + мобильный табельный учёт

Дата: 2026-09-07. ПЕРЕНОС одним инкрементом SW kipia-v419→v420
(сразу после проверки Task 334 пользователем в kip8test — заявка
охватывала и десктоп-кнопку «Вид», и мобильный учёт).

Состав Task 334 (детали — worklog kip8test@06fbbae):
1. КНОПКА «ВИД» — БЕЗ ЗНАЧКОВ: svg wsViewIconFull/Shift/Day удалены,
   только span.ws-view-label «Вид»; _updateViewBtn без иконок.
2. КАРТОЧКА ПРИБОРА: .dev-detail-top (картинка + «№ прибора» +
   «Место установки») — sticky (мобайл top 56px, десктоп-панель top 0
   + full-bleed −16px); фикс скролл-контейнера страницы прибора
   (overflow: visible — «мобильная защита» overflow-y:auto ломала
   липкость).
3. МОБИЛЬНЫЙ ТАБЕЛЬ: #wsCrossBtn скрыт ≤1023px; окна
   «Мероприятия»/«Нормы» изначально скрыты — раскрываются чипами
   .ws-mob-chips (toggleMobPanel).
4. ИТОГИ УЧЁТА на мобиле — СТРАНИЦА #page-ws-totals «Итоги учёта»
   (3 шеврона, вкладки, тело #wsTtPageBody, фамилии; toggleTotals →
   navigateTo; хуки onTotalsPageOpen/Close; _ttBodyEl; десктоп —
   шторка как прежде).
5. СУЖЕНИЕ ФАМИЛИЙ ДО 4 БУКВ при горизонтальной прокрутке (сетка +
   итоги): span.ws-emp-full/ws-tt-name + data-s4/_surname4;
   _narrowApply/_reapplyEmpNarrow/_attachEmpNarrow; CSS 76px/58px
   ≤1023px.

ПЕРЕНОС: патч index.html 737 строк (git apply ЧИСТО, изоляция 0);
sw.js kipia-v419→v420; tests/ — все 51 файл kip8test с адаптацией
версий (v573→v420, v574→v421); node --check OK. Тесты 2055→2087/0
(ПАРИТЕТ kip8test). Браузер на kip8 (порт 8945): task334 39/39
[мобайл-тёмная/светлая/десктоп], регресс [актуализированы]:
333 36/36, 315 33/33, 317 49/49, 328 36/36; 0 JS-ошибок.

Доки: scripts/task334-browser-check.py + актуализированные
task333/315/317/328-чеки; промт post-Task 334 (v420, 2087/51);
worklog. СЕРВЕРНОЙ ЧАСТИ НЕТ — Apps Script/листы НЕ трогать.
...
## Task 342 — перенос из kip8test (e8d95a4): колонка переработок в печатной форме

**Заявка пользователя:** «добавь колонку переработок» (к печати графика
работ — Task 341).

**Решение (перенос патча 122 строки ЧИСТО; только клиент):**

1. **Колонка «Перераб.»** — третья колонка итогов печатной шахматки:
   ДНИ/ЧАСЫ переработки кодов «д»/«н» (счётчики over/overDays из
   _totalsAgg — Task 322; формат годовой вкладки «3/36», часы с
   запятой _fmtTotalsNum; ноль — «—», сотрудника нет в agg — пусто).
2. **Шапка** — «Перераб.» + подпись «дни/ч» (wsp-tot-over 12mm,
   подпись 6.5px; бюджет A4 landscape 281,5 мм); **итоговая строка** —
   grand тем же форматом + подпись; **сноска** поясняет формат вместо
   отсылки в приложение. Экранная сетка/«Итоги учёта» не менялись.
3. **Тесты:** test-task342.js (17, версии kipia-v427/v428-guard),
   run-all.js регистрация; SW-бамп ассертов v426→v427 в 40
   тест-файлах, guard v427→v428; 2267 passed / 0 failed (паритет
   kip8test); node --check — 4 инлайн-скрипта ОК.
4. **Браузер:** task342-browser-check.py 43/43 (порт 8947; 5
   контекстов, 0 JS-ошибок; VLM: 3 колонки итогов, 1/12/—/1/4,
   grand 2/16, без обрезаний) + регресс 341 38/38 (E7 адаптирован:
   итоговых колонок 3).

**Деплой:** GitHub Pages + Ctrl+Shift+R ×1–2 (kipia-v427).

Следующий номер задачи: 343.

## Task 343 — перенос из kip8test (776e150): из печати убраны строка итогов и миниатюры иконок мероприятий

**Заявка пользователя:** «из печати убери строку итогов и миниатюры
иконок мероприятий».

**Решение (только клиент, перенос патчем index.html — 200 строк
ЧИСТО):**

1. **Итоговая строка «Итого» (wsp-sum, agg.grand) УБРАНА** из
   печатной формы _buildPrintHtml + CSS @media print. Печать
   заканчивается строками сотрудников с построчными итогами
   «Дни»/«Часы»/«Перераб.» (Task 342 — колонки остаются);
   grand-итоги — в приложении, вкладка «Месяц» «Итогов учёта».
2. **Бейджи мероприятий в ячейках печати (wsp-ev/wsp-ev-wrap/
   wsp-ev-plan) УБРАНЫ** из _printCell и CSS: _eventsAt не
   вызывается, виртуальный бейдж статус-мероприятия не строится;
   день с мероприятием печатается ПУСТОЙ ячейкой (мероприятия — в
   окне «Мероприятия в этот день» и разделе «Инструктажи»).
   ЭКРАННАЯ сетка не тронута — бейджи ws-ev-badge живут (Task 314).
3. **Сноска wsp-foot** без «бейдж в углу ячейки — мероприятие…»;
   пояснения плана отпуска/красной точки/«Перераб.» живы. Красная
   точка переработки, пунктирный план отпуска, серая заливка
   нерабочих, легенда кодов — НЕ тронуты.

**Верификация:** тесты +18 test-task343.js (версии kipia-v428) →
2282/0 (паритет с kip8test 776e150); node --check 4 инлайн-блока
ОК; браузер task343-browser-check.py 48/48 (5 контекстов, 0
JS-ошибок) + регресс 341 38/38 (E6/E14/V2 адаптированы) + 342
43/43 (E6/O6/O7/V2 адаптированы); VLM: итоговой нет, бейджей
мероприятий нет (красная точка переработки — задумана), 3 колонки,
ничего не обрезано.

**SW:** kipia-v427 → v428 (ассерты 41 файл, guard v428→v429 в 22).

**Деплой:** GitHub Pages + Ctrl+Shift+R ×1–2 (kipia-v428).

Следующий номер задачи: 344.

## Task 344 — фикс «общий вход» kip8 ↔ kip8test (восстановление прод index.html)

Дата: 2026-09-08. Заявка: «установил два приложения kip8 и kip8test
на один телефон, захожу в аккаунт в приложении kip8, затем…
открываю приложение kip8test, а оно почему-то тоже вошло в тот же
аккаунт, хотя на нём я не выполнял вход».

## Причина

Перенос Task 341 (коммит e50d394) скопировал в kip8 ТЕСТОВЫЙ
index.html ЦЕЛИКОМ — вместе с обёрткой isolateLocalStorage()
(префикс 'kip8test:' ко ВСЕМ ключам localStorage) и тест-ключами
kip8test_*. Оба PWA живут на одном origin
(bloknett-design.github.io), хранилище общее: kip8 стал писать
токен в 'kip8test:kip8_session_token' — ровно тот ключ, который
читает kip8test → вход в kip8 автоматически логинил kip8test.
Доказательство: md5 живых страниц совпадали (24a8b6f9…,
2 923 176 байт), sw.js при этом был корректный (прод, v428);
переносы 342/343 накладывались уже на испорченный файл. Урок:
перенос kip8test → kip8 обязан идти через
scripts/prepare-kip8-transfer.py (инструкция системного промта) —
теперь это проверяется тестом (test-task344.js), а не только
ручным grep-ом из чеклиста.

## Сделано (только kip8: index.html + sw.js + тесты)

1. **scripts/task344-fix-index.py** — восстановление по эталонному
   расхождению kip8@5f435e1 ↔ kip8test@2c40a87 (Task 340,
   ДО инцидента): удаление блока обёртки + 6 замен ключей/URL
   (kip8test_devices_cache/phonebook/DEV_CACHE-префиксы/
   #exam-tickets) + 6 прод-формулировок комментариев
   (Task 243/244 sidebar/overlay, «Сапёр», URL Apps Script,
   Task 242). Все 13 замен — со строгим контролем числа вхождений.
   Верификация восстановления: дифф с текущим kip8test
   СТРОЧНО-В-СТРОЧНО равен доинцидентному эталону (82 строки);
   легитимных упоминаний kip8test — 4 (как в 5f435e1); отличий от
   5f435e1 — 459 строк = только фичи печати 341–343.
2. **SW kipia-v428 → v429** (scripts/task344-bump-sw.py) —
   инвалидация кэша: без бампа телефоны продолжали бы получать
   испорченный index.html из кэша v428. Ассерты v428→v429 в 42
   тест-файлах (158), guard v429→v430 (27).
3. **tests/test-task344.js (+6)** — машино-исполняемый чеклист
   «после переноса kip8test → kip8»: 0 упоминаний
   isolateLocalStorage; нет 'kip8test:'/kip8test_*/'kip8test:#';
   VM: KipAuth.setToken пишет РОВНО 'kip8_session_token';
   SW = kipia-v429.

## Верификация

Тесты 2282→2288/0; node --check 4 инлайн-блока ОК;
task344-browser-check.py 18/18 — локальный сервер ОДНОГО origin
с путями /kip8/ и /kip8test/ (точное воспроизведение раскладки
Pages): вход в kip8 → kip8test в том же профиле ОСТАЛСЯ ГОСТЕМ,
raw-ключ kip8 есть в общем хранилище, но патч kip8test его не
видит; обратный сценарий (вход в kip8test) — kip8 тоже гость;
0 JS-ошибок. Регресс: task343-browser-check.py 48/48 (печать
341–343 и гейты 340 живы).

**SW:** kipia-v428 → v429.

**Деплой:** GitHub Pages + Ctrl+Shift+R ×1–2 (kipia-v429).
kip8-desktop подхватит CI-автосинк (sync-to-desktop.yml сработает
по index.html). kip8test (PWA) НЕ тронут — обёртка там НОРМА.

**Влияние на пользователей:** данные kip8, сохранённые ПОСЛЕ
деплоя Task 341 (7 сент., v426), попали в namespace 'kip8test:*' —
после фикса к kip8 вернутся старые данные (избранное/вид/тема,
сессии до 7 сент.); вошедшим в период инцидента придётся войти
заново. kip8test может остаться залогиненным «утёкшим» токеном —
нажать в нём «Выйти» один раз.

Следующий номер задачи: 345.

---

## Task 346 — перенос из kip8test (b0c2a64): политика сессий «1 моб + 1 десктоп»

**Заявка:** «с одной почты параллельно работать в мобильном и
десктопном приложениях; при входе в десктоп после мобильного — без
запрета "уже вошел"; но не больше двух входов, один — мобильный,
другой — десктопный.»

**Диагноз (снапшот KIP8_Access 08.09.2026):** параллельные входы
уже разрешены (блокировок нет), лимитов нет (12 сессий у одного
email), тип устройства не различался (колонки device в листе
sessions нет).

**Перенесено дельта-патчем** (109 строк index.html — только правки
Task 346, БЕЗ тестовой обёртки kip8test — урок Task 344; проверка:
0 isolateLocalStorage, 0 'kip8test:'):
- KipAuth._detectDeviceType() + device в payload verifyOTP + тост
  при evicted>0 + бейдж «моб»/«десктоп» в админ-панели;
- scripts/SessionsDevicePolicy.gs + DEPLOY-Task346-sessions-device-
  policy.md (сервер Apps Script: колонка device, вытеснение сессии
  того же типа, FAIL-OPEN);
- scripts/task346-browser-check.py (24/24) + task346-bump-sw.py.

**SW:** kipia-v429 → v430 (43 файла, guard v431).
**Тесты:** +18 (версии kipia) → **2306/0** (было 2288). Браузер:
24/24 (5 контекстов, 0 JS-ошибок, скриншоты).

**Активация сервера:** DEPLOY-Task346-sessions-device-policy.md —
вставить SessionsDevicePolicy.gs, sdpInit, 2 строки в
...
---
## Task 350 (09.09.2026) — замки на последние мутации + батч-удаления

Заявка: «Делай пункты 1+2 одним деплоем» (аудит после Task 349).
Изменены 2 файла: Utils.gs, Auth.gs (Sessions/Code/листы/клиенты не
тронуты). Деплой = 2 замены + «Новая версия».

- Utils.gs: крон-чистки cleanupExpiredSessions/cleanupExpiredOtpCodes/
  cleanupOldAuditLogs — под Utils.withLock (30 сек): крон бежит
  параллельно запросам юзеров, чужой deleteRow сдвигал номера строк →
  чистка могла удалить ЧУЖУЮ строку (класс бага Task 348);
  cleanupExpiredSessions читает users ОДИН раз (индекс живых ID вместо
  findUserById на каждую строку, O(n²) → O(n)); чистки audit_log и
  otp_codes срезают сплошной верхний блок одним deleteRows(5, N)
  (листы append-only), страховочный проход с конца для выбившихся из
  хронологии (row - prefix); incrementOtpAttempts ВОЗВРАЩАЕТ новое
  значение (атомарный read-modify-write под замком)
- Auth.gs verifyOTP: найдена и закрыта ДЫРА ДВОЙНОГО SUBMIT Task 348 —
  OTP читался ДО замка, два параллельных submit одного кода оба видели
  «unused», второй создавал ВТОРУЮ сессию. Теперь getActiveOtpForEmail
  ТОЛЬКО внутри замка (пере-чтение); ВСЕ проверки (истечение, неверный
  код, инкремент попыток) и мутации под ОДНИМ замком; гард удаления
  юзера внутри замка (null ≠ TypeError); аудит OTP_VERIFIED/
  LOGIN_SUCCESS внутри замка (appendRow не сдвигает чужие строки)
- Auth.gs sendOTP: [самосинхронизация (юзер перечитывается внутри) →
  блокировка OTP → кулдаун → генерация+appendRow] одним замком —
  закрыт TOCTOU кулдауна (2 письма/2 строки за 60 сек при параллельных
  запросах); MailApp по-прежнему ПОСЛЕ замка; гард удаления юзера
- Тесты: test-task350.js +33 (SRC-гарды позиций паттернов — с
  stripCommentLines, чтобы некрологи в комментариях не мешали; VM:
  батч/страховка на позиционном мок-листе, двойной submit — сессия
  ОДНА, кулдаун/блок/перебор email/удаление юзера, письмо вне замка,
  appendRow внутри); актуализирован test-task348 (sendOTP теперь С
  замком на секцию мутаций; verifyOTP — все markOtpUsed/getActive
  внутри) → **2427/0** (было 2394)
- DEPLOY-Task350-locks-cleanup-batch.md: таблица «мутация → проблема →
  стало», дыра двойного submit, батчи, проверки (кулдаун при двойном
...
Auth.gs: email-блок sendOTP УДАЛЁН (счётчик OTP_FAILED по email
контролировался знающим адрес жертвы: 5 мусорных verifyOTP → блок
выдачи кода на 30 мин, продлеваемо до бесконечности). Utils.gs:
countRecentOtpFails удалён (мёртв); ключ OTP_BLOCK_MINUTES не
читается. Защита брутфорса — на не-контролируемом атакующим:
MAX_OTP_ATTEMPTS=5 на код, TTL 10 мин, кулдаун 60 с, глобальный
100/час; неверные попытки НЕ сжигают код; после лимита — дешёвый
отказ без инкремента/аудита. Тесты +16 (test-task352.js) →
**2489/0**. Пуш: b11d3d8. GAS-деплой выполнен пользователем
(09.09.2026: Auth.gs + Utils.gs из download/task352/). DEPLOY-Task352
— в scripts/. Пп.7 (гигиена — НЕ-правка) и 8 (ip/ua-колонки —
отложено) закрыты решением аудита.

## Task 353 (2026-09-09): UI удаления пользователя в админ-панели
Клиент к серверному adminDeleteUser (Task 351, GAS уже развёрнут).
index.html: кнопка «Удалить» (.admin-delete-btn) в карточках
«Пользователи» — своя строка disabled (title-подсказка);
модалка #deleteUserOverlay (delete-user-*) с подтверждением —
email через textContent (XSS-безопасно), «Отмена»/«Удалить»,
ошибка сервера — В МОДАЛКЕ (гарды «себя»/«последнего админа» на
русском), кнопка блокируется на время запроса, после успеха —
тост с числом снесённых сессий + перезагрузка users/sessions;
фильтр журнала + опция ADMIN_DELETE_USER. SW kipia-v430→v431.
Перенос из kip8test через prepare-kip8-transfer.py (дополнен 5
комментариями-изолятами — первое полное применение; дифф
репо = эталонные 82 строки). Тесты test-task353.js +28 (SRC+VM:
модалка, ошибка, даблклик, renderUsers, регистронезависимость
isSelf) → **2517/0**; браузер 26/26 (happy/error/cancel/мобайл
320-390px, 0 JS-ошибок, VLM-скриншоты). Пуш: см. git log.
DEPLOY-Task353-admin-deleteuser-ui.md — в scripts/. GAS НЕ
трогался; десктопы — CI-синком.

Следующий номер задачи: 354.

## Task 355 (2026-09-09): перенос из kip8test — шахматка табеля, косметика
Заявка: в нерабочих выходных/праздничных ячейках убрать точку «·»;
линии ячеек и шапки (дни месяца) — тонкие 1px, но ярче; розовый
выходных пастельнее. Перенос через prepare-kip8-transfer.py — дифф
kip8test↔kip8 = эталонные 82 строки (изоляты), Task 355-дельта
78 строк. Изменения: _renderCell (dayOff ? '' : '·'); CSS —
светлая 30% чёрного, тёмная дни 30%/ФИО стале-синяя 55%, шапка
с вертикальными разделителями (border-top: 0, Task 331 жив),
гвард прозрачности «Сотрудник +» (Task 336 жив), розовый
#f7d9e3 → #f8e2e9 (обе темы). SW kipia-v431→v432 (45 файлов);
test-task355.js (kipia-v432/v433) + актуализация
test-work-schedule/test-task312/test-task319
(task355-transfer-tests.py) → **2540/0**; браузер 29/29 (порт
8954; праздник 08.09 — сеяный кэш '0908'; красные стыки Task 255,
полосы Task 259/333 живы). GAS НЕ трогался; десктопы — CI-синк.
DEPLOY-Task355 — в scripts/.

Следующий номер задачи: 356.

---
Task ID: 357
Task: ПЕРЕНОС из kip8test (f2d6912) + ПЕРЕНОМЕРОВКА: локальный Task 348 (расходомеры: дата «вчера» по умолчанию + красный «пора вводить») прошлой сессии не был запушен — параллельная сессия заняла номера 348–356; перенумерован в Task 357 и переложен поверх Task 356.
...
---
Task ID: 360–364 (перенос пакета)
Agent: Z.ai Code (главный)
Task: ПЕРЕНОС из kip8test в kip8 по прямому указанию пользователя (заявка Task 364: «И перенеси в боевой kip8») — пакет из 5 задач, проверенных пользователем в kip8test: 360 (печать: список мероприятий + перечень кодов месяца под шахматкой), 361 (колонка «Сотрудник» по ширине текста, крупные шрифты, бейджи мероприятий в ячейках), 362 (мероприятия только печатаемого вида + коды мероприятий в перечне, баг ПЗ), 363 (фон выходных = пустым, красная рамка-группа вокруг групп выходных, праздники розовые + даты), 364 (рамка 2px #e57373 спокойнее + коды на печати столбиком СПРАВА от столбика мероприятий).

Work Log:
- prepare-kip8-transfer.py (де-изоляция kip8test/index.html): дифф с kip8 = ровно зоны Tasks 360–364 (705 строк; print CSS @media print, экранные правила wgrp/feast, JS _buildPrintHtml/_printCell/_renderGrid/_renderCell, хелперы _calDayFeast/_calWend). Дефолтные изоляты не тронуты (кнопка .ws-dismiss-submit #e53935 жива). Де-изоляция применена: cp /tmp/kip8_index_transfer.html index.html.
- scripts/task360-364-port-tests.py: 11 тест-файлов (новые test-task360–364.js + адаптированные под Tasks 361–364 старые test-task319/341/342/343/355/work-schedule.js), версионный маппинг kipia-test-v593→v441, v594→v442, «нет захардкоженных версий» kipia-test-v59→kipia-v44; исторические guard-версии (v514/v517/…) оставлены в стиле kip8. run-all.js: +5 requires.
- SW-бамп kipia-v436→v441 (маппинг: v437=Task360, v438=361, v439=362, v440=363, v441=364): 49 тест-файлов (guards v437→v442, asserts v436→v441).
- Косметика из kip8test подтянута до переноса: 2 комментария _renderCell «3px»→«2px» (Task 364), тесты kip8test 2800/0.
- Прогоны kip8: тесты 2806/0 (было 2688, +118) с первого раза; браузер scripts/task360-364-browser-check.py 24/24 (порт 8962: сетка 2px #e57373 бока/тень/низ + грань 1px + фон выходного == пустому + праздник розовый с датой; печать wsp-bottom flex/row, коды правее мероприятий, ≤44% ряда, сноска под обоими, «ПЗ — Проверка знаний», 3 мероприятия; мобайл 375; 0 JS-ошибок); VLM по пруфам task360-364-proof-print/grid.png — 3/3 ДА.
- DEPLOY-Task360-364-transfer-from-test.md (таблица пакета, маппинг версий, откат).

Stage Summary:
- kip8 переведён с kipia-v436 (Task 359) на kipia-v441: весь пакет Tasks 360–364 в бою — печать табеля (мероприятия/коды по видам, «Сотрудник» по тексту, крупные шрифты, бейджи, коды справа от мероприятий) и экранная сетка (рамка-группа выходных 2px #e57373, праздники розовые + даты).
- Тесты 2806/0, браузер 24/24, VLM-пруф. GAS не тронут, десктопы — CI-синк.
- Оба репо синхронны по функциональности: kip8test = kipia-test-v593 (Task 364), kip8 = kipia-v441 (пакет 360–364).
- Следующий номер задачи: 365 (в обоих репо).

---
Task ID: 365 (перенос из kip8test)
Agent: Z.ai Code (главный)
Task: kip8 Task 365 — ПЕРЕНОС из kip8test (коммит 4a22d39, kipia-test-v594) по заявке пользователя «Перенос в боевой kip8»: расходомеры хозрасчётные — (1) №12 период «Ежедневно» (нормализация на клиенте во всех 3 источниках данных: таблица hozraschet_meters хранит устаревшее «Еженедельно», а форма ввода суточная); (2) красный «пора вводить» с 6:00 НОВЫХ суток (следующих за сутками ввода данных) ДЕРЖИТСЯ до ввода новых данных — поправка заявки «не ровно час» (логика Task 357 подтверждена и закреплена в комментариях _isOverdue/CSS и тестах).

Work Log:
- PAT обновлён на новый токен (github_pat_…UBYnKpGfXJ, действует с 2026-08-24, бессрочно); доступ к обоим репо подтверждён ls-remote (kip8 35eee49, kip8test 4a22d39 — оба синхронны с remote; «ahead 1» в status — устаревший tracking-ref после URL-fetch).
- prepare-kip8-transfer.py при 4a22d39 → дифф с kip8/index.html = ровно зона Task 365 (41 строка: комментарий CSS .flow-summary-val-due, 3 точки применения + определение _normalizeMeters, комментарий-поправка _isOverdue). Косметика Task 364 («3px»→«2px» в комментариях _renderCell) уже была в kip8 — перенос 364 шёл из рабочего дерева с хвостом.
- scripts/task365-port.py: cp index.html; data/flowmeters.json №12 «Еженедельно»→«Ежедневно» (+\n в конце); SW-бамп kipia-v441→v442 (guards СНАЧАЛА v442→v443, затем ассерты v441→v442 — урок Task 361; 54 тест-файла).
- Тесты: tests/test-task365.js перенесён с маппингом версий (kipia-test-v594→v442, v593→v441, v595→v443); run-all.js +1 require (80). Прочие изменения тестов в kip8test-коммите — только версии SW, покрыты бампом.
- Прогоны kip8: тесты 2838/0 (было 2806, +32) с первого раза; браузер scripts/task365-browser-check.py 24/24 (порт 8964; localStorage БЕЗ префикса изоляции — сырые ключи kip8_session_token/app-theme/kip8_flow_cache_v1; мок flowmeter.list с «Еженедельно» у №12 + кэш с тем же; патч даты subclass-Date): бейдж №12 «Ежедневно», №3 «Еженедельно»; №2/№12 зелёные, №4 красный; завтра 6:30 — №2/№12 красные; 7:30/12:00/послезавтра — держится (НЕ час); деталь №12 «Ежедневно»; форма — дата «вчера», №2 зелёный сразу после «Сохранить» и после серверного load(); мобайл 375; 0 JS-ошибок. VLM по пруфам task365-proof-desktop.png + form.png — 4/4 и 4/4 ДА.
- DEPLOY-Task365-transfer-from-test.md (таблица, серверная часть не требуется — нормализация идемпотентна, Apps Script не менялся; откат).

Stage Summary:
- kip8 переведён с kipia-v441 (пакет 360–364) на kipia-v442: №12 «Ежедневно» (нормализация на клиенте, fallback-json), красный «пора вводить» с 6:00 новых суток до ввода новых данных.
- Тесты 2838/0, браузер 24/24, VLM-пруф. GAS не тронут (период в Google Sheets можно поправить руками — поведение не изменится).
- Оба репо синхронны по функциональности: kip8test = kipia-test-v594 (Task 365), kip8 = kipia-v442 (Task 365).
- ⚠️ PUSH В GITHUB НЕ ВЫПОЛНЕН: PAT из сообщения пользователя отклонён (API 401 Bad credentials, push 401 Invalid username or token — на оба репо, все схемы). Ранее рабочий токен из /home/z/.kip_pat был перезаписан и не восстановим. Коммит 3c6f072 ждёт валидный PAT (Contents: Read and write) — инструкция в DEPLOY-Task365-transfer-from-test.md, раздел «PUSH НЕ ВЫПОЛНЕН».
- Следующий номер задачи: 366 (в обоих репо).

---
Task ID: 366–369-перенос
Agent: Z.ai Code (главный)
Task: ПЕРЕНОС пакета Tasks 366–369 из kip8test (5e6e9ec) в боевой kip8 (после Task 365, c230d07) — по заявке пользователя «Перенос в боевой kip8»: 366 (server_busy повторяем + серверный дедуп архива; баннер → далее убран), 367 (жёлто-оранжевые значения недоставленных + календарные периоды), 368 (ввод №3/№11/№9 за период двух дат + красный по ЗАКРЫТЫМ неделям/месяцам), 369 (свайп «Проекты» → «Проекты по статусу»).

Work Log:
- kip8test Task 369 доработан и запушен ранее в этой сессии: коммит 5e6e9ec (до amend 5dce0d7), SW kipia-test-v598, CI success (CI Tests + Deploy Pages + Sync + pages build), живой SW подтверждён curl. Worklog-запись Task 369 в kip8test добавлена; браузер-чек починен 26/26 (селекторы: .page-content.active binding из-за ленивого рендера страниц, хлебные крошки через endsWith, годы через #projectGroupList .pb-section-title-text, Python sorted() вместо list.sort() возвращающего None); VLM desktop 5/5, light 4/4, mobile 5/5.
- Перенос: prepare-kip8-transfer.py при 5e6e9ec → дифф 518 строк, проверен пофрагментно (выборочные hunks 17144/37069/38328 + свод) — только зоны 366–369. scripts/task366-369-port.py: (1) cp index.html + маркеры 366/367/368/369; (2) SW kipia-v442→v446; (3) базовый бамп 55 тестов (guard v443→v447, ассерты v442→v446); (4) перезапись 6 адаптированных + 4 новых тестов из kip8test с маппингом v599→v447/v598→v446/v596→v445/v595→v444/v594→v442/v593→v441 (исторические v534/v537/v585/v586 оставлены — соглашение kip8); (5) run-all.js +4 require. Контроль leftovers — чисто.
- scripts/FlowmeterArchive.gs скопирован 1:1 из kip8test (зона Task 366: _normEntryType/_isDuplicateArchiveRow + appendToArchive под Utils.withLock; node --check через .js-копию — валиден). Code.gs и прочие .gs НЕ переносились: маршруты функционально идентичны (множества case-маршрутов совпали, расхождения только в комментариях). data/flowmeters.json идентичен — не трогали.
- Тесты kip8: 2977/0 с первого прогона (2838 + пакет).
- Браузер scripts/task366-369-browser-check.py 31/31 (порт 8969): десктоп — цвета по закрытым периодам (№2 зелёный/№4 красный/№3 зелёный закрытая неделя/№11 красный старее/№9 зелёный прошлый месяц/№5 «Ежемес.» красный), подпись-диапазон №3 «за 07.09–13.09.2026 г.», баннера недоставленных НЕТ, форма №3 «Период с/по» = закрытая неделя 07–13.09, реальный ввод №11 → payload datePrev/dateCurr = границы закрытой недели → карточка зелёная, offline-ввод №4 → жёлто-оранжевые значения #f5a623 → после _flushOutbox снова зелёные; проекты: кнопка в свайп-ячейке, подложки «По статусу» ×2, свайп влево → projects-status с группами Новый/Выполнен/Остановлен/Отменен/Действующий/(без статуса) и счётчиками 2/3/1/1/1/1, группа «Выполнен» → 3 карточки с годами 2019/2020/2024, тап → projects-prod, свайп вправо → тоже status; мобайл 375 — свайп + группы + расходомеры; 0 JS-ошибок. Починки чек-скрипта: closeDetailPanel() вместо FlowmeterData.closeDetail() (в kip8 нет такого метода); AD переведена на нетронутые №3/№5 (мок-стейт общий, №11/№4 обновлены вводом контекста 1).
- VLM: desktop 4/4 (заголовок, порядок, счётчики, без дефектов), mobile 5/5.
- DEPLOY-Task366-369-transfer-from-test.md (таблица пакета с маппингом SW v595-v598→v443-v446; серверная часть — FlowmeterArchive.gs уже обновлён пользователем в Apps Script вручную, подтверждено в чате; откат).

Stage Summary:
- Пакет Tasks 366–369 В БОЕВОМ kip8 (SW kipia-v446): баннер недоставленных заменён жёлто-оранжевым цветом значений (#f5a623/#c96e00, приоритет над красным) с возвратом зелёного после доставки; недельные/месячные — красный по календарной неделе/месяцу; показания №3/№11/№9 вводятся за ПЕРИОД двух дат (дефолт — закрытая неделя/прошлый месяц), красный по ЗАКРЫТЫМ периодам, подписи-диапазоны; свайп кнопки «Проекты» в КИП ИОС открывает «Проекты по статусу» (тап — «По отделениям»).
- Тесты kip8 2977/0, браузер 31/31, VLM-пруфы; kip8test 2971/0, браузер 26/26 (Task 369).
- Сервер: FlowmeterArchive.gs (дедуп Task 366) — пользователь уже обновил в Apps Script («Новая версия»); в репо kip8 файл синхронизирован для тестов.
- Следующий номер задачи: 370 (в обоих репо).

---
Task ID: 370
Agent: Z.ai Code (главный)
Task: kip8 Task 370 (синхронно с kip8test) — заявка пользователя: «внеси изменения сразу и в тестовый и в боевой проекты, жёлто-оранжевые значения недоставленных вместо баннера, измени цвет чуть ярче и ближе к желтому.» Прямой коммит в боевой, без стадии приёмки на тестовом.

Work Log:
- CSS index.html (тот же дифф, что в kip8test): .flow-summary-val.flow-summary-val-pending — #f5a623 → #ffc400 (тёмная) / #c96e00 → #cc9900 (светлая); комментарий-маркер Task 370. Только цвет — логика (приоритет pending над красным, зелёный после доставки, закрытые периоды Task 368) не тронута.
- SW kipia-v446→v447 (scripts/task370-bump-sw.py; 59 тест-файлов: guard v447→v448, ассерты v446→v447; v443=366…v446=369, v447=370). tests/test-task370.js +9 (kip8-версия: SW v447/guard v448). Адаптация test-task367.js (ассерты цвета → #ffc400/#cc9900). run-all.js +1 require. → 2986/0 (2977+9).
- Браузер scripts/task370-browser-check.py 26/26 (порт 8971, localStorage БЕЗ префикса изоляции): новый амбер rgb(255,196,0)/rgb(204,153,0); полный регресс Task 367/368 с детерминированными моками закрытых периодов (урок kip8test: «TODAY-7» всегда накрывает закрытую неделю); offline-ввод → цвет сам → флаш → зелёный; светлая; мобайл; 0 JS-ошибок.
- VLM: desktop «ярко-золотистый, ближе к жёлтому» (№2/№4), light, mobile — палитра подтверждена.
- DEPLOY-Task370-pending-color-brighter-yellow.md (серверных действий НЕ требуется; откат git revert).

Stage Summary:
- Task 370 В БОЕВОМ kip8 (SW kipia-v447): недоставленные показания — значения ярко-золотистые #ffc400/#cc9900 (было #f5a623/#c96e00). Тесты 2986/0, браузер 26/26, VLM-пруфы.
- kip8test синхронно: SW kipia-test-v599, тесты 2980/0, браузер 26/26.
- Следующий номер задачи: 371 (в обоих репо).

---
Task ID: 371–374-перенос
Agent: Z.ai Code (главный)
Task: Перенос пакета Tasks 371–374 из kip8test в kip8 по заявке Task 374: «…И переноси все изменения в боевой kip8». Пакет: 371 — блок «Расчёт произвольных значений» (1693706); 372 — переделка раздела «Датчики температуры» на карточки + страница датчика (fb920ee); 373 — мобильные карточки столбиком, избранное TempFav, бейджи ТС/ТП убраны, термопара ТХК (L), панель над формой (c75739f); 374 — кнопка «Копировать» над таблицами значений убрана (018a999, первая часть той же заявки).

Work Log:
- prepare-kip8-transfer.py → /tmp/kip8_index_transfer.html (блок isolateLocalStorage удалён; kip8test_*→kip8_* 8 замен; 5 комментариев-изолятов). Дифф с kip8/index.html = 562 строки; все ханки — зоны датчиков температуры (CSS .ts-* ~1146–1209; разметка страниц ~16874–16934; реестры navigateTo hash-редирект/PAGE_PARENTS/PAGE_LABELS/топ-бар/списки ролей; JS ~30006–30533 — каталог/TempFav/openTempSensor/calcTempSensor/справка; саблейбл меню «Термометры сопротивления и термопары»); проверено пофрагментно, посторонних изменений нет. cp → kip8/index.html.
- sw.js: CACHE_VERSION kipia-v447 → kipia-v451 (v448=371, v449=372, v450=373, v451=374).
- scripts/task371-374-port-tests.py: перенесены test-task371/372/373/374.js (маппинг kipia-test-v603→kipia-v451, guard v604→v452, «v604 ещё не существует»→«v452 ещё не существует»; захардкоженных kipia-test-v60 в файлах нет); test-role-access.js — адаптированная версия из kip8test (temp-sensors → страница карточек, расчёт — на temp-sensor-view; других отличий от kip8-версии нет — проверено диффом, версионных ссылок нет); базовый бамп 60 файлов (ассерты v447→v451: 236, guard-ы v448→v452: 44; исторические v441…v446 не тронуты). run-all.js — +4 require точечно (у kip8 свои комментарии, файл не копировался целиком).
- Тесты: node tests/run-all.js → 3096 passed / 0 failed с первого прогона (2986 + 110 тестов пакета: 32+31+35+12).
- Браузер scripts/task371-374-browser-check.py 36/36 (порт 8976; десктоп 1280 тёмная / светлая / мобайл 375; localStorage БЕЗ префикса изоляции; 0 JS-ошибок): карточки 8 ТС + 9 ТП (ТХК (L) хромель-копель −200…800); табы «Все 17 / Избранные 0»; бейджей на карточках нет; звёзды 17; клик ☆ → ★, «Избранные 1», фильтр, скрытие пустой группы; ТХК (L) — панель «Расчёт произвольных значений» НАД формой (видна сразу), подпись «Термо-ЭДС E(t), мВ», живой расчёт E(100)=6,862, звезда в шапке; «Рассчитать» — таблица E(t) БЕЗ кнопки «Копировать» (кнопок в результатах 0), E(100)=6,8617; 50М — R(t), R(50)=60,7, без кнопки; «Шкала-сигнал» — своя кнопка «Копировать» на месте (соседние разделы не тронуты); светлая — ТХА (K) без кнопки, E(100)=4,096; мобайл — карточки столбиком по одной на всю ширину. Подводный камень: контейнер табов — класс .ts-tabs БЕЗ id (tempSensorsTabs не существует) — счётчики через #tsAllCount/#tsFavCount. VLM по 3 скриншотам: таблицы без «Копировать», карточки столбиком на всю ширину.
- scripts/DEPLOY-Task371-374-transfer-from-test.md (состав пакета таблицей, маппинг версий, откат).

Stage Summary:
- Пакет Tasks 371–374 ПЕРЕНЕСЁН в боевой kip8 (SW kipia-v451): раздел «Датчики температуры» — карточки 17 датчиков (8 ТС + 9 ТП с ТХК (L)), страница датчика со статичной панелью «Расчёт произвольных значений» над формой диапазона/шага, избранное TempFav (табы + звёзды, localStorage kip8_temp_fav_v1), мобильные карточки столбиком, бейджей ТС/ТП на карточках нет, кнопки «Копировать» над таблицами значений нет (соседние разделы не тронуты).
- Тесты 3096/0 (с первого прогона), браузер 36/36, VLM-пруфы ×3. Серверных действий НЕ требуется (чистый клиентский калькулятор).
- kip8test — источник пакета (018a999, SW kipia-test-v603, тесты 3078/0…3090/0), CI success, десктоп-зеркало автосинкнуто.
- Следующий номер задачи: 375 (в обоих репо).

---
Task ID: 375-перенос
Agent: Z.ai Code (главный)
Task: ПЕРЕНОС Task 375 из kip8test (78412e8, kipia-test-v604) в боевой kip8 — по заявке пользователя «Внеси все изменения в боевой kip8»: (1) печать табеля — столбец кодов справа от столбца мероприятий на расстоянии 10px; (2) хозрасчётные расходомеры — усиленная защита от случайных автоматических дублирований новых записей (в момент ввода и в первый час окна правки).

Work Log:
- Проверено окружение: PAT валиден (API /user → 200 ДО записи, урок Task 365); оба репо синхронны с remote (kip8test 018a999 → после Task 375 push 78412e8; kip8 af805de).
- kip8test Task 375 выполнен ранее в этой сессии: печать gap 5mm→10px; анти-дубли клиент (№1 кулдаун 2.5 c _inputAcceptedAt / №2-№3 _isDuplicateReadingInput meters+outbox с ключом без prev / №4 _outboxCollapseDuplicates в _flushOutbox до серверных дедупов) + сервер (справочная копия FlowmeterArchive.gs: правило «окно 1 часа» — тот же meterId+curr+dateCurr+entryType ≤1 ч назад по O-таймштампу = дубль независимо от prev; точный ключ Task 366 сохранён). Тесты kip8test 3132/0 (+42), браузер 30/30 (порт 8977), VLM 5/5, CI success (Tests/Pages/sync-to-desktop), живой прод kipia-test-v604.
- ПЕРЕНОС: prepare-kip8-transfer.py → де-изоляция → дифф с kip8/index.html = 116 строк — ТОЛЬКО зоны Task 375 (wsp-bottom gap 10px + комментарии; submitInput кулдаун/дубль-блок/флаг; _isDuplicateReadingInput; openInput сброс; _submitPeriodEntry флаг; _flushOutbox свёртка; _outboxCollapseDuplicates), проверено пофрагментно. cp → kip8/index.html.
- scripts/FlowmeterArchive.gs: gs-файлы kip8 и kip8test были ИДЕНТИЧНЫ до Task 375 (diff git show HEAD обоих = пусто) → скопирована пропатченная версия (дифф +20/−5 = только правило «окно 1 часа» + комментарий); node-синтаксис валиден; Flowmeter.gs НЕ менялся.
- sw.js: CACHE_VERSION kipia-v451 → kipia-v452 (scripts/task375-bump-sw.py: guard-ы v452→v453 сначала, ассерты v451→v452 затем; 64 тест-файла; повторный запуск корректно упёрся в защиту двойного бампа).
- Тесты: test-task375.js перенесён с маппингом kipia-test-v604→kipia-v452, guard v605→v453, «старая» kipia-test-v603→kipia-v451; адаптации test-task364.js (gap 5mm→10px), test-task366.js («Другое prev» — фикстура O 2-часовой давности: свежий случай теперь дубль по правилу 2, покрытие в test-task375), test-task358.js (flushMixin + _outboxCollapseDuplicates); run-all.js +1 require. node tests/run-all.js → 3138 passed / 0 failed с первого прогона (3096+42).
- Браузер scripts/task375-browser-check.py 30/30 (порт 8978; localStorage БЕЗ префикса изоляции — сырые ключи kip8_session_token/app-theme, урок Task 365; десктоп/печать/мобайл 375; 0 JS-ошибок): реальный ввод №2 — ровно 1 updateReading (payload prev=383291, curr=383400.5, дата=вчера); двойной вызов submitInput() подряд — вторая отправка НЕ ушла, payload.prev НЕ сдвинут дважды (383400.5, не 383500); повтор того же значения за ту же дату — отправки нет, тост «уже введены… Изменить показания (окно 1 час)»; свёртка пары «оригинал+двойной сдвиг» в outbox — уходит только оригинал (prev исходный 679700); правка isEdit — проходит (3-я отправка, isEdit=true); печать: wsp-bottom flex-ряд, gap computed=10px, коды правее мероприятий ровно на 10px (rect ±1.5), оба столбика от общего верха, сноска под обоими на всю ширину, регрессы 360/362/364; мобайл — печать и двойной ввод живы. VLM: структурный пруф (2 колонки, выровнены по верху, узкий зазор, каждый код строкой, сноска внизу, без дефектов) + прицельный кроп нижнего блока: зазор МЕНЬШЕ высоты строки (согласуется с 10px; программные computed/rect — авторитетны).
- scripts/DEPLOY-Task375-transfer-from-test.md: состав переноса таблицей, маппинг версий, ⚠️ серверная часть — ручное «Новая версия» FlowmeterArchive.gs в Apps Script (клиент работает и без неё), откат.

Stage Summary:
- Task 375 ПЕРЕНЕСЁН в боевой kip8 (SW kipia-v452): печать табеля — коды справа от мероприятий на расстоянии ровно 10px; анти-дубли расходомеров — 4 клиентских слоя + серверное правило «окно 1 часа» в справочной копии FlowmeterArchive.gs.
...
---
Task ID: 377-transfer
Agent: Z.ai Code (главный)
Task: kip8 — перенос Task 377 из kip8test 395d5b3 (табель: усиление подсветки текущего дня + полосы ячеек светлой темы цветом тёмной).

Work Log:
- Перенос scripts/task377-patch.py → kip8: 12 правок index.html; ДИФФ побайтово совпадает с Task 377 kip8test (161+/20−, diff-сравнение) — только зоны 377, автокоммиты данных не затронуты.
- SW kipia-v453→v454 (scripts/task377-bump-sw.py; 66 тест-файлов: ассерты v453→v454, guards v454→v455).
- Тесты: test-task377.js перенесён с маппингом kipia-test-v606→kipia-v454 / v607→v455 (+чистка остаточных сообщений v605/v607 в 363/377); адаптации test-task313/319/355/363.js — скопированы из kip8test (проверено: нетривиальных kip8-отличий нет, только версии и значения 377); run-all.js +1 → 3183/0 с первого прогона (3164+19).
- Браузер scripts/task377-browser-check.py 28/28 (порт 8982, localStorage БЕЗ префикса — урок 365; обе темы; 0 JS-ошибок): computed заливки/шапки/полоса/границы; hover/select не гасит; красная рамка ws-wgrp-first жива; ПИКСЕЛЬНО линии совпадают между темами (9,15,23)~(10,15,23) обе ориентации; тинт виден. VLM: столбец 17 заметно выделен, линии тёмные, красные рамки видны. DEPLOY-Task377-transfer-from-test.md.

Stage Summary:
- Task 377 В ПРОДЕ kip8: усиление «сегодня» (заливки ×2, жирное число, полоса 2px, не гаснет при наведении/выборе) + полосы светлой темы = цвета тёмной (дни rgb(10,15,23), ФИО rgb(64,80,102), шапка rgb(83,96,117)).
- SW kipia-v454; тесты 3183/0; браузер 28/28; VLM-пруф. Серверных шагов НЕТ (чистый CSS-клиент).
- Следующий номер задачи: 378.

---
Task ID: 381-transfer
Agent: Z.ai Code (главный)
Task: Перенос Task 381 из kip8test e2eddb0 в боевой kip8 (по действующей заявке «переносить все изменения в боевой kip8»).

Work Log:
- kip8 синхронен (9696d00 = Task 380, SW kipia-v457); все 9 якорей task381-patch.py проверены в kip8/index.html ДО запуска.
- scripts/task381-patch.py (копия kip8test) → 9 правок; дифф 90+/37− — только зоны Task 381 (побайтово та же правка, что в kip8test).
- scripts/task381-bump-sw.py: sw.js kipia-v457→v458 (guard v459); 70 тест-файлов (ассерты v457→v458, guards v458→v459).
- Тесты Task 381: test-task381.js + адаптации test-task378/379/322/321/380 скопированы из kip8test с маппингом kipia-test-v610→kipia-v458 / v611→v459; diff с sed-нормализацией версий — отличий НЕТ (kip8-специфики в этих зонах нет); run-all.js +1 → 3270/0 с первого прогона.
- Браузер scripts/task381-browser-check.py 63/63 (порт 8991; localStorage БЕЗ префикса — kip8_session_token/app-theme, урок 365; обе темы + мобайл 375; 0 JS-ошибок): C — склейка ≤0.75px/краяд/стык зон/от заголовка 5px; B — значок НА МЕСТЕ (120px/колесо/конец/возврат, transform, _barExpPin обоих окон, раскрытие высота=scrollHeight+маржа 95−H); A — итоги (прозрачные ячейки, зебра 0.09/0.07, базовые Часы/Переработка, линии Task 378, нулей нет, панель #e9e7de, шахматка #FFFFFF жива); M — мобайл (склейка/края/прикол/итоги через wsTtPageBody).
- Пиксельные пруфы kip8 (идентичны kip8test): тёмная — past (43,50,60)/upcoming (7,12,18) на окне (14,22,33) — зоны НЕПРЕРЫВНЫ; светлая — (245,244,240)/(204,203,195) на (233,231,222); итоги — нечёт = фон панели / чёт = зебра (35,43,53)/(216,214,206) — НЕ белый.
- DEPLOY-Task381-transfer-from-test.md; эта worklog-запись; коммит + push (PAT в URL → сброс после пуша).

Stage Summary:
- Task 381 В ПРОДЕ обоих репо: kip8test e2eddb0 (kipia-test-v610, 3264/0) и kip8 (kipia-v458, 3270/0). Итоги учёта — фон «как до белого» (прозрачные ячейки + зебра Task 322; линии/нули/перекрестье Task 378 и шахматка #FFFFFF Task 379 живы); значки раскрытия окон мероприятий/норм ПРИКОЛОТЫ к видимой нижней грани при прокрутке текста; окно «Мероприятия» — ОБЩИЙ ФОН всего пространства окна на уровне текста (строки до краёв + склейка одно-сроковых, стык только между прошедшими и идущими-будущими).
- Серверных шагов НЕТ (чистый клиент). Десктоп-зеркала синхронизирует CI.
- Следующий номер задачи: 382.

---
Task ID: 382-transfer
Agent: Z.ai Code (главный)
Task: Перенос Task 382 из kip8test 811d963 в боевой kip8 (по действующей заявке «переносить все изменения в боевой kip8»).

Work Log:
- kip8 синхронен (b938f1d = Task 381, SW kipia-v458); база для переноса проверена: дифф kip8test@e2eddb0 ↔ kip8@HEAD по index.html = 82 строки РЕПО-СПЕЦИФИЧНЫХ различий (префиксы kip8test:/isolateLocalStorage, комментарии, URL Apps Script) — зоны Task 382 идентичны.
- Перенос: хунки извлечены из диффа kip8test e2eddb0→811d963 (git diff, 6 хунков с контекстом 3 — якоря уникальны), применены к kip8/index.html; scripts/task382-patch.py сгенерирован из тех же хунков (labels добавлены); ПРОВЕРКА воспроизводимости: git checkout → запуск скрипта → diff с прямым переносом = IDENTICAL; финальный дифф kip8 ↔ kip8test = те же 82 репо-специфичные строки (побайтовая точность зон).
- scripts/task382-bump-sw.py: sw.js kipia-v458→v459 (guard v460; 71 тест-файл: guards v459→v460 СПЕРВА, затем ассерты v458→v459); повторный запуск — dup-защита без записи.
- Тесты: test-task382.js (+24) скопирован с маппингом kipia-test-v611→kipia-v459 / v612→v460; адаптации test-task325/327/329 скопированы (зоны Task 382: 42% → var(--ws-tt-emp-w, 42%) + маппинг версий; дифф с HEAD = только адаптация + бамп); run-all.js +1 → 3294/0 с первого прогона.
- Браузер scripts/task382-browser-check.py 33/33 (порт 8993; localStorage БЕЗ префикса — kip8_session_token/app-theme, урок 365; обе темы + мобайл 375 + десктоп 1280; 0 JS-ошибок): результаты ИДЕНТИЧНЫ kip8test — ширина колонки месяца 210px = переменной = натуральный текст 192+18; «Год» ≈ месячной ±3px; возврат восстанавливает; сужение живо; 0 клиппинга; равные данные; десктоп скрыт; пиксельный пруф границы на th.right rgb(64,80,102).
- DEPLOY-Task382-transfer-from-test.md (kip8/scripts); эта worklog-запись; коммит + push (PAT в URL → сброс после пуша).

Stage Summary:
- Task 382 В ПРОДЕ обоих репо: kip8test 811d963 (kipia-test-v611, 3288/0) и kip8 (kipia-v459, 3294/0). Мобильная страница итогов, вкладка «Месяц» — ИЗНАЧАЛЬНАЯ ширина колонки фамилий ПО ШИРИНЕ ТЕКСТА в ячейках (переменная --ws-tt-emp-w, мерит _measureTtEmpFullW: № таб. + 5 маржи + ФИО + заголовок + 16 + 2; фолбэк 42%) — совпадает с вкладкой «Год» (±3px); fixed-раскладка/равные данные/сужение при прокрутке/плавность 0.35s/десктоп — НЕ тронуты.
- Серверных действий НЕТ (чистый клиент). Десктоп-зеркала синхронизирует CI (sync-to-desktop).
- Следующий номер задачи: 383 (в обоих репо).

---
Task ID: 383
Agent: Z.ai Code (главный)
Task: Подготовка проектов к переходу в новый чат — синхронизация «Системного промта» боевого kip8 (заявка: «подготовь файлы проектов для перехода работы над проектом в новом чате»).

Work Log:
- Состояние проверено: kip8 @bd8e37e (SW kipia-v459, 3294/0, 95 файлов, ~49.6 тыс. строк), kip8test @811d963 — деревья чистые, Tasks 340–382 в проде обоих репо.
- «Системный_промт_для_приложения_КИПиА.md» был версии 2026-09-08 post-Task 339 — отставание на 43 задачи.
- НОВАЯ шапка-версия 2026-09-18 post-Task 382 (с точки зрения боевого репо): перенесённое за период 340–382 (детали — в шапке kip8test-версии), ОТКРЫТЫЕ серверные действия Task 375/376 (ручная замена FlowmeterArchive.gs И Flowmeter.gs в Apps Script, инструкции DEPLOY-Task375/376-transfer-from-test.md; клиент работает и без них; Task 366 развёрнут подтверждён), урок PAT (API /user до записи в /home/z/.kip_pat).
- Метрики: ~49.6 тыс. строк/~3.1 MB, 93 страницы, 3294 теста/95 файлов, ожидание run-all 3294/0, примеры kipia-v459→v460, WEB_APP_URL ~строка 32 100, номера строк модулей (KipAuth ~32 094, KipCableJournal ~35 101, KipFav ~33 743, FlowmeterData ~36 967, +НОВЫЙ WorkSchedule ~40 227).
- «Последняя принятая партия» → Tasks 375–382 (источники 78412e8→811d963, SW kipia-v452→v459, паритет 3294/0).
- Коммит 0af291d + push (PAT → сброс URL, leak-check 0); копия в /home/z/my-project/download/Системный_промт_для_приложения_КИПиА_kip8.md.
...
---
Task ID: 385-389-transfer
Agent: Z.ai Code (главный)
Task: Перенос Tasks 385–389 из kip8test в боевой kip8 одним кумулятивным коммитом (команда пользователя: «Перенеси все изменения в боевой kip8»). Состав: 385 — страница «Работники» (полные карточки CRUD + кнопка в баре, карточка шахматки read-only, «работник» во всех строках, шторка «Легенда»); 386 — «Обозначения» (кнопка/два вида шторки/мобильная страница, надпись шапки, «Добавить работника»); 387 — панель до 500px, канон справочника кодов (порядок+полные наименования, нормализация на клиенте), пояснения −3/«красная рамка», «Выходной» без точки; 388 — сплошные миниатюры с цветом кода, краткие обозначения, итоги в сменном/дневном виде, значок раскрытия в правом верхнем углу, вкладки «Работников» столбиком слева; 389 — прошедшие мероприятия = общий фон окна, кнопка и численность (авто + штат) на «Общей» вкладке, сплошные фоны ярлыков/окон, левый край.

Work Log:
- PAT проверен API /user (200, bloknett-design) ДО записи; kip8 чист (HEAD 128cab0 = Task 384-transfer, SW v460, core.fileMode false — фантомных mode-изменений нет).
- Перенос: последовательное применение scripts/task{385,386,387,388,389}-patch.py (копии kip8test, итоговая реплика 5952d60 — task389-patch.py в канонической форме) против kip8 — ВСЕ якоря совпали с первого прохода (385: 51 правка index.html; 386: 26; 387: 17 index.html + 1 WorkSchedule.gs + 2 StatusCodesInit.gs; 388: 42; 389: 14 + 2 канонических фикса: empty-state одной строкой-литералом, комментарий «авто-центрирование» без литерала margin: 0 auto).
- scripts/task385-389-transfer.py: тесты/ синхронизированы из kip8test (102 файла — все адаптации 385–389 + новые test-task385..389.js) с маппингом версий kipia-test-v617→kipia-v465 / guard v618→v466; run-all.js — восстановлен kip8-специфичный require('./test-task344.js') после task343; test-task344.js — бамп v460→v465; sw.js — CACHE_VERSION kipia-v460→kipia-v465.
- Контроль портов/данных: дифф kip8↔kip8test по index.html = 55 репо-специфичных строк (было 82 на Task 384 — часть репо-специфичных комментариев переписана задачами нейтрально; зоны 385–389 идентичны побайтово); sw.js дифф — только суффикс -test; WorkSchedule.gs/StatusCodesInit.gs — правки Task 387 идентичны.
- Тесты 3518/0 с первого прогона (3512 kip8test + 6 kip8-специфичных test-task344; единственный остаточный «kipia-test-v603» — guard в test-task375, истинный на kip8).
- Браузер: scripts/task388-browser-check.py 61/61 + scripts/task389-browser-check.py 37/37 (порт 9001 — python3 -m http.server в каталоге kip8; адаптация localStorage БЕЗ префикса: kip8_session_token/app-theme, кэш kip8_ws_cache_v1; результаты идентичны kip8test: обозначения краткий 230px/развёрнутый 500px, «*»-бейдж сплошной #FFAB91 на пустом и «.»-дне, итоги сменного/дневного/полного вида (шторка живёт при смене вида), значок раскрытия в правом верхнем углу (верх ±2px при раскрытии), вкладки «Общая»+Иванов→Петров→Сидорова, карточка/возврат; Task 389: прошедшие → rgba(0,0,0,0), текущие → 0.45/0.12, окно #0e1621/#E9E7DE, ПРЕДЫДУЩИЙ месяц — все прозрачные (детерминизм), «Работники»: в шапке кнопки НЕТ, «На текущий момент: 3 работника (автоматический подсчёт).» + «По штату: 14, из которых 2 мастера, 5 сменных и 7 дневных.», кнопка ВНУТРИ «Общей» (клик → шторка «Новый работник»), ярлыки #1a2233/#0e1621 СПЛОШНЫЕ, панель .ws-wgen, margin 0 + отступ 12px, мобайл: лента #E4E0D3/окно #F0EEE6; 0 JS-ошибок ×5 контекстов).
- DEPLOY-Task385-389-transfer-from-test.md: серверные шаги за пользователем — Task 384 Apps Script (ОТКРЫТ ранее; обязателен для правок карточек) + Task 387 ОПЦИОНАЛЬНО (клиент самодостаточен) + Task 375/376 проверить; ОДИН бэкенд на оба сайта. Коммит + push (PAT → сброс URL, leak-check 0).

Stage Summary:
- kip8 В ПРОДЕ: Tasks 385–389 перенесены (SW kipia-v465; страницы 96 → +ws-workers/+ws-legend; тесты 3518/0, браузер 61/61 + 37/37 — идентичны kip8test). Оба сайта синхронизированы: kip8test kipia-test-v617 ↔ kip8 kipia-v465, диффы — только репо-специфика.
- ⚠️ ОТКРЫТО (за пользователем): сервер Apps Script вручную — Task 384 (Code.gs 2 case + WorkSchedule.gs updateEmployee/updateVacation + «Новая версия»; до деплоя «Сохранить» правок вернёт Unknown action) и Task 387 ОПЦИОНАЛЬНО (getStatusCodes + StatusCodesInit.gs канон; нормализация уже на клиенте). Task 375/376 (Flowmeter) — проверить.
- Следующий номер задачи: 390 (в обоих репо).

---
Task ID: 390-transfer
Agent: Z.ai Code (главный)
Task: Перенос Task 390 из kip8test в боевой kip8 (заявка: строки штата/текущего момента по категориям, выделенная шапка, примыкание ярлыков, светлые тёплые ярлыки в тёмной теме).

Work Log:
- PAT (новый, от 24.08.2026) проверен (200, bloknett-design); kip8 чист (HEAD 77e90e9 = Tasks 385–389, SW v465).
- Перенос: scripts/task390-patch.py против kip8 — ВСЕ 11 якорей совпали (зоны Task 390 побайтово идентичны kip8test; дифф зоны CSS вкладок = 0 строк). scripts/task390-transfer.py: бамп тестов (guard v466→v467, ассерты v465→v466 — 78 файлов; ПОРЯДОК: бамп ДО копии test-task390, иначе generic-бамп испортил бы её v466), sw.js kipia-v465→v466, копия tests/test-task390.js (маппинг kipia-test-v618→kipia-v466 / v619→v467), run-all.js require test-task390 (после task389; kip8-специфичный task344 не тронут — бампнулся generic-ом v465→v466).
- ../kip8test/scripts/task390-adapt-tests.py в корне kip8 — 17 правок (task385 ×2, task388 ×6, task389 ×9; якоря совпали — файлы отличаются от kip8test только версиями).
- Тесты 3540/0 (3518 + 22 task390). Браузер scripts/task390-browser-check.py 24/24 (порт 9003, kip8-вариант: localStorage БЕЗ префикса kip8test: — результаты идентичны kip8test). VLM (тёмная kip8): шапка выделена, ярлыки тёплые контрастные, примыкание, сноски нет.
- Коммит 2df8aa0 + push (PAT → сброс URL, leak-check 0); CI ×4 success (Tests/Sync-to-desktop/Build Desktop App/pages build); ПРОД curl: SW kipia-v466, все маркеры Task 390 в живом index.html, старых формулировок («На текущий момент:», «По штату: 14, из которых», ws-wgen-note) — 0. DEPLOY-Task390-transfer-from-test.md; копии в download/.

Stage Summary:
- БОЕВОЙ kip8 ОБНОВЛЁН ДО Task 390 (SW kipia-v466): «Общая» вкладка «Работников» — «Работников по штату 14: 2 мастера; 7 дневных; 5 сменных.» + «Работников на текущий момент: N мастера; N дневных; N сменных.» (автоподсчёт по категориям, мастера = должность «Мастер КИПиА», исключены из типов); сноска удалена; шапка выделена (#1E2B42/#E4E0D3 + рамка/радиус); ярлыки примыкают к окну (padding-right убран); тёмная тема — тёплые светлые ярлыки #4B4E46/#575A50/#63665B.
- Тесты 3540/0, браузер 24/24, CI ×4 success. Серверных шагов НЕТ; открытые серверные за пользователем прежние (Task 384 Apps Script обязателен для правок карточек; Task 387 опционален).
- Следующий номер задачи: 391 (в обоих репо).

---
Task ID: 391-transfer
Agent: Z.ai Code (главный)
Task: Перенос Task 391 из kip8test ae70fc5 в боевой kip8 (строки шапки «Общей» вкладки в скобках «(N) ((N) мастера…)», шрифт крупнее/ярче, кнопка «Добавить работника» акцентная в стиле сайта).

Work Log:
- PAT проверен (200, bloknett-design); kip8 чист (HEAD 64d4bb8 = Task 390-transfer, SW v466); авто-коммитов поверх не было (fetch → ahead 1 без rebase).
- Перенос: ../kip8test/scripts/task391-patch.py — 7/7 якорей (зоны 385–391 побайтово идентичны); scripts/task391-transfer.py (бамп 78 тестов v466→v467/guard v467→v468 ПОРЯДОК: guards первыми; sw.js kipia-v466→v467; test-task391.js маппинг v619→v467/v620→v468; run-all.js + require после task390); ../kip8test/scripts/task391-adapt-tests.py — 17 правок (task390 ×8, task389 ×6, task386 ×1, task388 ×2).
- Тесты 3556/0 (3540 + 16 task391). Браузер scripts/task391-browser-check.py 20/20 (порт 9005, localStorage БЕЗ префикса; результаты идентичны kip8test 9004: тексты, 14px/600/0.95, кнопка rgb(74,143,199)/rgb(198,97,63) по теме, шторка «Новый работник», инварианты Task 390, 0 JS-ошибок ×3); VLM ×1.
- Коммит 3ccd9c6 + push (PAT→сброс URL, leak-check 0); CI @3ccd9c6: CI Tests / Sync content to kip8-desktop / pages build — success; Build Desktop App — зеркалит десктоп автоматически. ПРОД curl: живой SW = kipia-v467, все маркеры Task 391 (totalN, «(2 мастера, 7 дневных, 5 сменных).», var(--accent-blue), brightness(1.12), padding 0 14px), старых формулировок 0.
- DEPLOY-Task391-transfer-from-test.md (включая статус серверных: Task 384 — ВЫПОЛНЕН пользователем; Task 387 — опционален); копии в download/.

Stage Summary:
- БОЕВОЙ kip8 ОБНОВЛЁН ДО Task 391 (SW kipia-v467): строки шапки «Общей» вкладки «Работников» — формат в скобках («Работников по штату 14 (2 мастера, 7 дневных, 5 сменных).» + «Работников на текущий момент (N) ((N) мастера, (N) дневных, (N) сменных).»), шрифт 14px/600/0.95, кнопка «Добавить работника» акцентная (тёмная — синий #4a8fc7, светлая — оранжевый #C6613F, белый текст). Серверных шагов НЕТ (один бэкенд Apps Script не затронут).
- Тесты 3556/0, браузер 20/20, CI success. Десктоп-зеркало — CI Sync success.
- Следующий номер задачи: 392 (в обоих репо).

---
Task ID: 392-transfer
Agent: Z.ai Code (главный)
Task: Перенос Task 392 из kip8test в боевой kip8 (заявка: строка «Работников на текущий момент 12 (…)» — 12 без скобок; раздел «СИЗ» — лист «СИЗ» табель_КИП_ИОС с редактированием/добавлением/удалением из личных данных каждого работника, образец — «Таблица СИЗ работникам КИП ИОС.xlsx»).

Work Log:
- PAT проверен (200, bloknett-design); kip8 чист (3ccd9c6 = Task 391, SW v467); висячие worklog-записи Task 391 закоммичены отдельным коммитом 93c48e7 (worklog+DEPLOY).
- scripts/task392-patch.py (копия из kip8test) — 18/18 якорей с первого прохода: index.html ×11 (строка без скобок, CSS .ws-ppe-*, шторка #wsPpeSheet, _PPE/_loadPpe/кэш c.ppe, секция карточки, JS-функции), WorkSchedule.gs ×5 (эндпоинты/листы/структура/PPE_SHEET/CRUD), Code.gs ×1 (4 case), PPEInit.gs создан.
- scripts/task392-transfer.py: sw.js kipia-v467→v468; тесты — guards v468→v469 ПОПЕРВУ, затем ассерты v467→v468; tests/test-task392.js — копия из kip8test с маппингом v620→v468/v621→v469; контентные адаптации (task391 ×5, task389 ×3, task390 ×5 вкл. SRC totalN, task388 ×2, test-tab-numbers _appendRowKeepText 4→5, test-task314 loadGrid(true) 15→18, run-all.js require).
- Тесты 3603/0 (3556 + 47 Task 392; kip8-специфичный task344 жив). Браузер task392-browser-check.py 28/28 (порт 9007, localStorage БЕЗ префикса kip8test:) — результаты идентичны kip8test (строка без скобок; секция СИЗ с записями/кнопками; шторка с авто-датой; add/update/delete; мобайл; 0 JS-ошибок ×2). node --check WorkSchedule.gs/PPEInit.gs/Code.gs ✓.
- Коммит + push (PAT→сброс URL, leak-check 0); CI ×4 success; ПРОД: SW kipia-v468, маркеры Task 392 в живом index.html. DEPLOY-Task392-transfer-from-test.md + копия DEPLOY-Task392-ppe-sheet-workers-count.md (серверные шаги); пробы task392-proof-*.png; копии в download/.

Stage Summary:
- БОЕВОЙ kip8 ОБНОВЛЁН ДО Task 392 (SW kipia-v468): строка текущего момента с числом БЕЗ СКОБОК; раздел «СИЗ» (лист «СИЗ» табель_КИП_ИОС, карточка работника ✎/✕/+ СИЗ, шторка, АВТО-дата окончания = выдача + срок). Оба сайта синхронны (диффы — только репо-специфика).
- ⚠️ За пользователем (ОДИН бэкенд Apps Script на оба сайта): серверный деплой Task 392 — Code.gs (4 case) + WorkSchedule.gs (замена целиком) + PPEInit.gs (ppeDeployAll — создаёт лист «СИЗ» и заполняет стандартный перечень 8 позиций каждому активному работнику) + «Новая версия». До деплоя раздел СИЗ деградирует мягко (пусто, тосты с Unknown action).
- Следующий номер задачи: 393 (в обоих репо).

---
Task ID: 393
Agent: Z.ai Code (главный)
Task: БОЕВОЙ kip8 — перенос Task 393 из kip8test cb4f6b0: заявка «В картах работников нужно сделать шрифт текста больше, и разбить карты работников на четыре блока» (профиль с «Правка данных…»/«Уволить…» / «Отпуска · год» / «Мероприятия · месяц год» / «СИЗ · средства индивидуальной защиты»).

Work Log:
- PAT проверен API /user (200, bloknett-design) ДО записи; kip8 чист (HEAD a53ab8b = Task 392, SW v468; на remote — 3 авто-коммита CI-синка данных, ребейз).
- scripts/task393-patch.py — 20/20 якорей с первого прохода: index.html JS (сигнатура _renderWorkerCard(tabNo, withEdit, asBlocks), «не найден» → массив, b1–b4 блоки, возврат asBlocks ? [b1,b2,b3,b4] : склейка, НОВЫЙ метод _renderWorkerCardPanels — каждое блок ОТДЕЛЬНЫМ окном .ws-wcard, _renderWorkersPage зовёт панели) + CSS (шрифт КРУПНЕЕ каскадом .ws-wcard: шапка 11→15px, поля/строки 12→14px, метка 108→128px, заголовок блока 10→12px c border-bottom, «нет…» 11→13px, СИЗ 12→14px/мета 10.5→12.5px, ✎/✕ 22→26px; паддинг панели 14×16; стек .ws-wtab-body .ws-wcard 12px / :last-child 0). Попап #wsEmpPopup НЕ внутри .ws-wcard — прежняя типографика.
- scripts/task393-transfer.py: адаптация 6 тестов (385 ×4 вкл. HOST_METHODS + счётчик окон 1→4, 388 ×2, 389/390/391/392 — мок-панели _renderWorkerCardPanels с CARD-строкой в обёртке .ws-wcard); tests/test-task393.js — копия из kip8test с маппингом kipia-test-v621→kipia-v468 (ассерты) / v622→kipia-v469 (guards); run-all.js require; sw.js kipia-v468→v469 (guards v469→v470 ПОПЕРВУ, затем ассерты v468→v469; 82 файла).
- Тесты 3627/0 (3603 + 24 Task 393). Браузер scripts/task393-browser-check.py 19/19 (порт 9007, localStorage БЕЗ префикса): ЧЕТЫРЕ окна .ws-wcard, состав блоков по заявке, изоляция секций, шрифты 15/14/12/12.5px, зазор 12px, попап сетки — прежние 12px/11px сплошной, «Общая» — 0 окон, мобайл 375 в границах, 0 JS-ошибок ×2.
- Коммит acfe9c1 (ребейз на 3 авто-коммита) + push; CI ×4 success (Tests / Sync-to-desktop / Build Desktop App / pages build); ПРОД: SW kipia-v469, маркеры Task 393 в живом index.html (raw + Pages). DEPLOY-Task393-transfer-from-test.md; пробы task393-proof-*.png; копии в download/.

Stage Summary:
- БОЕВОЙ kip8 ОБНОВЛЁН ДО Task 393 (SW kipia-v469): карточка работника страницы «Работники» — ЧЕТЫРЕ блока-окна (профиль с действиями / отпуска / мероприятия / СИЗ) + шрифт крупнее; попап карточки у сетки и «Общая» вкладка — НЕ изменились. Оба сайта синхронны (диффы — только репо-специфика). СЕРВЕРНЫХ ШАГОВ НЕТ (чистый клиент; Apps Script не трогался).
- ⚠️ За пользователем остаётся серверный деплой Task 392 (Code.gs + WorkSchedule.gs + PPEInit.gs → ppeDeployAll → «Новая версия») — для работы раздела СИЗ в бою.
- Следующий номер задачи: 394 (в обоих репо).

---
Task ID: 394-transfer
Agent: Z.ai Code (главный)
Task: Перенос Task 394 из kip8test в боевой kip8 (зеркально коммиту 8d11ff7): сетка 2×2 карточки работника на весь экран (отпуска справа профиля, СИЗ под отпусками, мероприятия под профилем) + мероприятия за весь год в карточках (_loadTrainings без month) + окно мероприятий месяца с секциями «Отпуска» (частично на два месяца — в окнах ОБЕИХ) и «СИЗ» (срок истекает в месяце).

Work Log:
- PAT проверен (200, bloknett-design) ДО записи; kip8 чист (4764398 = Task 393-transfer, SW v469). Контекст: серверный деплой Task 392 ВЫПОЛНЕН пользователем; Task 394 — чистый клиент, серверных шагов нет (listTrainings без month — исходная способность сервера).
- scripts/task394-patch.py (копия из kip8test) — 11/11 якорей с первого прохода: index.html kip8 в зоне правок побайтово совпадает (сетка .ws-wgrid2 + кап 1020px снят; _loadTrainings { year } без месяца; карточка b3 «Мероприятия · ГОД» с фильтром года; _renderWorkersPage обёртка; _renderWorkersGeneral колонка-год; _renderMonthEventsPanel секции «Отпуска»/«СИЗ»; CSS плашки/точки секций).
- scripts/task394-adapt-tests.py — 4 правки (test-task393: «Мероприятия · 2026» ×2 + «нет мероприятий за год» + имя теста; test-task309: пустое состояние года).
- scripts/task394-transfer.py: копия tests/test-task394.js из ../kip8test с маппингом kipia-test-v622→kipia-v469 (ассерты) / v623→kipia-v470 (guards) — состояние «до бампа»; регистрация в run-all.js; sw.js kipia-v469→v470 (guards v470→v471 первыми, 83 файла).
- Тесты 3654/0 (3648 из kip8test + 6 kip8-специфичных test-task344).
- Браузер scripts/task394-browser-check.py 25/25 (порт 9007, localStorage БЕЗ префикса): 3 секции окна мероприятий; отпуск через границу В ОКНАХ СЕНТЯБРЯ И ОКТЯБРЯ; СИЗ «до 12.09»/«до 05.10»; день 12 (накрывающий отпуск + СИЗ ровно в день); «Работники»: display grid, равные колонки, отпуска СПРАВА профиля, СИЗ ПОД отпусками, мероприятия под профилем, «Мероприятия · 2026» (записи марта/июня/сентября); «Общая» — колонка-год (счётчик 3); мобайл 375 — стек, чип «Мероприятия» с секциями; 0 JS-ошибок ×2.
- Коммит b371d5e + push (PAT → сброс URL, leak-check 0); CI ×4 success (Tests / Sync content to kip8-desktop / Build Desktop App / pages build); ПРОД kip8: SW kipia-v470 (CACHE_VERSION), все маркеры Task 394 в живом index.html. DEPLOY-Task394-transfer-from-test.md; пруфы task394-proof-*.png; копии в download/.

Stage Summary:
- БОЕВОЙ kip8 ОБНОВЛЁН ДО Task 394 (SW kipia-v470): сетка 2×2 карточки на весь экран + мероприятия за год в карточках + окно мероприятий месяца с отпусками (частично на два месяца — в обоих) и СИЗ (срок истекает в месяце). Оба сайта синхронны (диффы — только репо-специфика). СЕРВЕРНЫХ ШАГОВ НЕТ (Apps Script не трогался; Task 392 серверный деплой выполнен пользователем — открытых шагов нет).
- Следующий номер задачи: 395 (в обоих репо).

---
Task ID: 395-transfer (+395)
Agent: Z.ai Code (главный)
Task: Перенос Task 395 из kip8test в БОЕВОЙ kip8 (коммит kip8test febce3e): кнопка «Работники» по матрице доступа (скрыта null/min), фон/рамки блоков карточек («выступающий бордюрчик»), десктоп-колонки (отпуска под профилем, мероприятия под отпусками, СИЗ в верхней правой части).

Work Log:
- Окружение: worklog прочитан (Task 394 в проде, следующий номер 395); PAT проверен API /user (200, bloknett-design) до записи; kip8 чист (ee01cf6, SW kipia-v470).
- Перенос кода: scripts/task395-patch.py (копия из kip8test, 7/7 якорей — index.html JS+CSS зеркально: init/_onRoleUpdate null+min, гейты openWorkersPage/_renderWorkersPage edit/view, панели колонками .ws-wcol/.ws-wcol-ppe, CSS #243349/#ffffff + 2px рамка + светлая верхняя кромка + тень, @media ≥1024px flex-колонки).
- Перенос тестов: scripts/task395-adapt-tests.py (копия, 5/5: test-task311 гейт edit/view, test-task385 SRC-гейты + _onRoleUpdate null/min + VM-зритель пускается/min/null мимо, test-task394 CSS flex-колонки); scripts/task395-transfer.py (копия test-task395.js с маппингом kipia-test-v623→kipia-v470 ×2 + kipia-test-v624→kipia-v471 + имя «SW поднят до kipia-v470»; регистрация в run-all.js; копия браузерной проверки: порт 9007, тег t395k8, localStorage БЕЗ префикса kip8test:, пруфы task395k8-*); scripts/task395-bump-sw.py v470→v471 (guards v471→v472 первыми, 84 файла; свой тест НЕ исключён — он в «до-бамп» форме, двухшаговая замена приводит в каноническую).
- Тесты: 3672/0 (3654 + 18 новых test-task395.js).
- Браузер task395-browser-check.py 31/31 (порт 9007, 5 контекстов — edit/view/min/null(«КИП8»)/мобайл-светлая): кнопка по матрице, read-only у зрителя, пустое тело прямого URL у min/null, экран «нет доступа» при null, колонки (профиль→отпуска→мероприятия слева, СИЗ верх справа, равные flex-колонки gap 12), стили (#243349/#ffffff, 2px, кромка ярче боков, тень 0 3px), мобильный стек, 0 JS-ошибок ×5.
- Деплой: коммит + push (ребейз на CI-авто-коммиты при необходимости); CI ×4 success; ПРОД SW kipia-v471 + все маркеры Task 395; DEPLOY-Task395-transfer-from-test.md (СЕРВЕРНЫХ ШАГОВ НЕТ).

Stage Summary:
- Task 395 В ПРОДЕ боевого kip8 (SW kipia-v471): кнопка «Работники» НЕ отображается пользователям без доступа к табелю (null) и с ограниченным просмотром (min) — по матрице; блоки карточек — фон светлее фона окна + рамки 2px «выступающим бордюрчиком»; десктоп — две колонки (отпуска под профилем, мероприятия под отпусками, СИЗ — верхняя правая часть); мобайл — стек. Тесты 3672/0; браузер 31/31; CI ×4. Серверных шагов НЕТ.
- ОБА САЙТА синхронны по Task 395 (kip8test SW kipia-test-v623 / kip8 SW kipia-v471).
- Следующий номер задачи: 396 (в обоих репо).

---
Task ID: 396-transfer (+396)
Agent: Z.ai Code (главный)
Task: Перенос Task 396 из kip8test 6826330 в боевой kip8 (заявка: зебра строк блоков карточек; компактные кнопки правки/увольнения и «+ …» в верхнем правом углу шапок блоков; оглавления блоков — шрифт крупнее/ярче + другой фон). Локальная часть переноса — БЕЗ push (PAT утрачен с откатом песочницы в начале цикла; прод обоих сайтов на GitHub цел — Task 395).

Work Log:
- Окружение: kip8 восстановлен git fetch + reset --hard origin/main (f62854c = Task 395, SW v471, база 3672/0 не прогонялась отдельно — совпадение с kip8test подтверждено идентичностью файлов зон 390–395).
- Перенос кода: scripts/task396-patch.py (копия kip8test) — 19/19 якорей с первого прохода; контроль синтаксиса inline-скрипта (new Function) OK.
- Перенос тестов: scripts/task396-adapt-tests.py (копия, 4 ассерта test-task393 + 1 test-task318); scripts/task396-transfer.py (копия test-task396.js с маппингом kipia-test-v623→kipia-v471/v624→v472/v625→v473 — пост-бамп форма; run-all.js require; копия браузерной проверки — порт 9007, localStorage БЕЗ префикса kip8test:, тег t396k8, пруфы task396k8-*); scripts/task396-bump-sw.py v471→v472 (guards v472→v473 первыми, 85 файлов, свой тест исключён).
- Подводный камень (найден браузер-чеком и исправлен): transfer-скрипт заменял СКЛЕЕННОЕ значение localStorage-блока (которого нет в исходнике скрипта) — setItem session token оставался с префиксом kip8test: → приложение не видело логина → кнопка «Работники» hidden; фиксы: исходнико-уровневые замены литералов + строгий контроль (в attach не осталось setItem('kip8test:…')); после фикса — клик и все проверки прошли.
- Тесты: 3691/0 (3672 + 19 перенесённых test-task396.js).
- Браузер scripts/task396-browser-check.py 32/32 (порт 9007, 3 контекста — тёмная edit/view/мобайл-светлая): результаты идентичны kip8test (4 шапки #1E2B42 во всю ширину, ФИО 16px/700, секции 13px/700/капс; 5 кнопок 26px: rgb(74,143,199)+rgb(239,83,80) в профиле, «+ …» в блоках, правый край у рамки; зебра 2/4-х строк всех блоков, radius 6px; легаси-строк внизу 0; попап прежний; view — шапки+зебра без кнопок; мобайл — #E4E0D3/#C6613F/#c62828/0.05, перенос кнопок ниже заголовка справа; 0 JS-ошибок ×3). VLM ×1 (glm-5v-turbo — подтверждено).
- Деплой: коммит b1af7fa создан ЛОКАЛЬНО; push заблокирован утратой PAT — прод остаётся на SW kipia-v471 до переотправки PAT пользователем (затем push → CI ×4 → проверка маркеров ws-whead/ws-wbtn/ws-row-alt). DEPLOY-Task396-transfer-from-test.md (СЕРВЕРНЫХ ШАГОВ НЕТ); контрольная задача: prod-маркеры после push.

Stage Summary:
- kip8 ЛОКАЛЬНО (коммит b1af7fa, SW kipia-v472): Task 396 перенесён из kip8test побайтово (зебра строк блоков, компактные кнопки в шапках, оглавления ярче с другим фоном); тесты 3691/0; браузер 32/32; VLM ×1. Серверных шагов НЕТ.
- ⚠️ БЛОКЕР (оба репо): PAT утрачен с откатом песочницы — kip8test (6826330+e484d0a) и kip8 (b1af7fa) ждут переотправки PAT пользователем для push → CI → прод (SW kipia-test-v624 / kipia-v472).
- Следующий номер задачи: 397 (в обоих репо, после push Task 396).

---
Task ID: 397-transfer
Agent: Z.ai Code (главный)
Task: Перенос Task 397 из kip8test 771f014 в боевой kip8 (ОБЩЕЕ ПРАВИЛО: кнопка раздела без доступа не отображается — на всё приложение).

Work Log:
- kip8 чист (ae0aa12 — Task 396, SW kipia-v472); дифф kip8↔kip8test(до-397) по index.html 82 строки — зоны патча не затронуты, test-role-access.js идентичен.
- scripts/task397-patch.py (копия kip8test) — 3/3 правки зеркально; task397-adapt-tests.py — 2 правки; task397-transfer.py — test-task397.js в «до-бамп» форме (v625→v472 ассерт, v626→v473 guard) + run-all.js; task397-bump-sw.py v472→v473 (guards v474 первыми, 87 файлов, свой тест не исключён — двухшаговый бамп канонизирует, паттерн task395).
- Тесты 3707/0 (3691 + 14 перенесённых + 2 адаптированных).
- Браузер task397-browser-check.py 35/35 (порт 9007, localStorage БЕЗ префикса kip8test:; 6 контекстов как в kip8test; пруфы task397k8-proof-*.png).
- Коммит + push; CI; ПРОД SW kipia-v473; DEPLOY-Task397-transfer-from-test.md (СЕРВЕРНЫХ ШАГОВ НЕТ).
...
---
Task ID: 399-transfer
Agent: Z.ai Code (kip8 — боевой)
Task: Перенос Task 399 из kip8test в kip8: «если у пользователя нет доступа к просмотру информации мастеров в табеле учёта, то и в окне мероприятий не должно быть информации мастеров (мероприятия, отпуска, СИЗ)».

Work Log:
- Окружение: worklog прочитан; kip8 на Task 398 (0f60a55, SW v474); PAT API /user 200.
- scripts/task399-patch.py — копия из kip8test (5 якорей проверены ДО запуска — все по 1 вхождению; зона патча побайтово идентична kip8test после применения).
- scripts/task399-transfer.py: tests/test-task399.js в «до-бамп» форме (маппинг kipia-test-v627→v474 ×2 / v628→v475 ×1) + регистрация в run-all.js (после 398).
- scripts/task399-bump-sw.py v474→v475 (guards v476 первыми, 89 файлов; свой тест НЕ исключён — двухшаговый бамп канонизирует, паттерн task395/397/398). Прогон 3732/0.
- Браузер task399-browser-check.py 33/33 ×4 контекста (порт 9007, localStorage БЕЗ префикса kip8test:, пруфы task399k8-*): min — сетка 2 строки/окно без мастера/счётчики «· 1»/день 7 пуст от мастеров/«Работники» скрыта; view/edit — полное «· 2»; мобайл 375 светлая min — чип, мастера нет; 0 JS-ошибок ×4. VLM ×2 (glm-5v-turbo: min-пруф + ПРИЦЕЛЬНЫЙ КРОП тулбара — первый VLM ошибочно «увидел» кнопку «Работники», кроп ряда 1 показал только селекты+«Обновить»+«Печать» — JS-проверка hidden+rect 0×0 авторитетна).
- Деплой: push отклонён (remote автокоммит 3d2fd38 «update cables data» — data/cables.json, вне зоны патча) → git rebase FETCH_HEAD → 43a6084; тесты после ребейза 3732/0; push OK; CI ×4 success (CI Tests / pages build / Sync content to kip8-desktop / Build Desktop App); ПРОД SW kipia-v475 + маркеры Task 399 (5); DEPLOY-Task399-events-window-masters-min.md (СЕРВЕРНЫХ ШАГОВ НЕТ); эта worklog-запись.
- Десктоп-зеркало kip8-desktop синхронизировано CI автоматически (Sync content).

Stage Summary:
- БОЕВОЙ kip8 (SW kipia-v475, коммит 43a6084) — Task 399 В ПРОДЕ: окно мероприятий табеля у уровня min не показывает записи мастеров (мероприятия/отпуска/СИЗ — _isMasterKipia как у сетки); edit/view — полное. Тесты 3732/0; браузер 33/33; VLM ×2+кроп; CI ×4; серверных шагов НЕТ.
- ОБА САЙТА теперь на Task 399: kip8test SW kipia-test-v627 + kip8 SW kipia-v475.
- Следующий номер задачи: 400 (в обоих репо).
...
Work Log:
- Состояние проверено: kip8 @51daa9e (SW kipia-v475, guard v476; тесты 3732/0, 113 файлов; index.html 52 141 строк / ~3.2 MB; 95 страниц; живой прод https://bloknett-design.github.io/kip8/ ; десктоп kip8-desktop v2.1.8), kip8test @95ad62d — деревья чистые, Task 399 в проде обоих репо.
- «Системный_промт_для_приложения_КИПиА.md» был версии 2026-09-18 post-Task 382 — отставание на 16 задач (384–399).
- scripts/task400-prepare-prompt.py (строгие якоря: шапка новая+прежняя→«предыдущая» + 22 точечных + 1 ручная правка примера CACHE_VERSION): НОВАЯ шапка-версия 2026-09-23 post-Task 399 с точки зрения боевого репо — текущее состояние (kip8 @51daa9e / kip8test @95ad62d, SW v475/v627, тесты 3732/0 и 3726/0, ~52.1 тыс. строк, десктоп 2.1.8) + сводка перенесённого 384–399 (детали — в шапке kip8test-версии) + ОТКРЫТЫЕ серверные действия (375/376 — статус у пользователя; 384/392/366 задеплоены подтверждённо) + уроки PAT (365/396).
- Метрики актуализированы: ~52.1 тыс. строк / ~3.2 MB; 95 страниц; 3732 теста / 113 файлов; ожидание run-all 3732/0 (kip8test: 3726/0); примеры бампов kipia-v475→v476 / kipia-test-v627→v628; маркеры в шапке (Текущая версия кэша kipia-v475) и таблица репо (kip8 kipia-v475 / kip8test kipia-test-v627 — оба были сильно устаревшими: v424/v577 с Task 339); позиции модулей (KipAuth ~33 019, KipFav ~34 742, KipCableJournal ~36 100, FlowmeterData ~37 966, WorkSchedule ~41 232); WEB_APP_URL ~строка 33 025.
- Секция Apps Script: состав .gs-файлов выровнен с фактическим scripts/ (+ PPEInit.gs и др.); эндпоинты workSchedule.* дополнены (updateEmployee/updateVacation, PPE CRUD, dismissEmployee, getPatterns, getStatusCodes; гейты view/view.min/edit). Секция «Защита _applyRoleToUI()»: + пункты 5 (ОБЩЕЕ ПРАВИЛО Task 397) и 6 (семантика hidden Task 398).
- «Последняя принятая партия» → Tasks 384–399 (источники c0ed8d7→5952d60→99464ba→ae70fc5→442d1a4→cb4f6b0→8d11ff7→febce3e→6826330→771f014→90a024c→35c68c2, SW kipia-v459→v475 позадачно, паритет 3732/0 от 2026-09-23; серверные 375/376 — вручную пользователем).
- История версий промта сохранена (post-Task 382 → «предыдущая», глубже — без изменений); устаревшие числа остались ТОЛЬКО в строках истории и исторических записях переносов (осознанно).
- Коммит + push (PAT → сброс URL, leak-check 0); копия промта в /home/z/my-project/download/Системный_промт_для_приложения_КИПиА_kip8.md.

Stage Summary:
- Промт боевого kip8 синхронизирован к post-Task 399 (версия 2026-09-23) — готов как контекст для нового чата (в связке с kip8test-версией как основной). КОД НЕ МЕНЯЛСЯ (только .md + scripts/task400-prepare-prompt.py), SW kipia-v475 без изменений, деплой не нужен. Серверных шагов НЕТ.
- Следующий номер задачи: 401 (в обоих репо).
---
Task ID: 401-404 (перенос в kip8)
Agent: Z.ai Code (kip8)
Task: Заявка (финал): «…И затем перенеси все последние изменения в боевой kip8» — перенос задач 401 (годовая шторка итогов: полная ширина правее ФИО + подстрока «дней/часов» + итоговые столбцы), 402 (тип работника третьей строкой ячейки + столбец «группа_допуска»), 403 («разряд» → «р.» и без группы в столбце ФИО; попап без СИЗ; GAS-фикс бага комментария; ярлыки по тексту; мероприятия над СИЗ), 404 (три колонки карточки: профиль+отпуска | СИЗ | мероприятия; кнопки ✎/✕ рядом; окно мероприятий без отпусков) из kip8test (0f39a94 → 9dfd4b6 → cae2b6c → a038911).

Work Log:
- Подготовка: kip8 подтянут (HEAD 4dab936, SW kipia-v475, Task 399 — последний перенос); базы сверены: kip8/index.html@4dab936 ↔ kip8test/index.html@c0ff6fe (пред-401) = 82 строки репо-специфики (localStorage-ключи kip8_/kip8test:, isolate-блок, комментарии Task 243) — ВНЕ зон патчей; WorkSchedule.gs — 0 различий (побайтово). Тесты-базы идентичны кроме актуальных версий SW (v475/v476 vs v631/v632) и исторических прод-комментариев; исторические guards kipia-test-v5XX — ОБЩИЕ для обоих репо.
- scripts/task401-404-transfer.py: 1) патчи 401 (9 правок index.html) → 402 (19 index + 8 GS) → 403 (13 index + 7 GS) → 404 (7 index) — применены чисто к kip8 (порядок обязателен); 2) 16 контентно-изменённых тестов скопированы из kip8test@HEAD с маппингом kipia-test-v631→kipia-v475, v632→kipia-v476 (ассерты/guards до-бамп-формы kip8; test-deploy-url.js и прочие чисто-версионные НЕ копировались — версии kip8 свои); 3) run-all.js — точечная вставка 4 require после test-task399.js (файл НЕ копировался: в kip8 kip8-специфичный test-task344.js и другие формулировки).
- ПОДВОДНЫЙ КАМЕНЬ: task403-patch.py в kip8test содержал СТАРЫЙ вызов this._fitWorkersTabs(); (VM-безопасный try/if я вписал в kip8test вручную ПОСЛЕ патча — в архив скрипта не вернул); в kip8 после применения патча тесты упали «_fitWorkersTabs is not a function» → вызов заменён на try { if (this._fitWorkersTabs) ... } catch (e) {} и в kip8/index.html, и в kip8test/scripts/task403-patch.py (архивная целостность для будущих переносов).
- scripts/task401-404-bump-sw.py: SW kipia-v475 → v476 (двухшаговый: guards v476→v477, затем ассерты v475→v476 — перенесённые тесты канонизированы: ассерты v476, guards v477; 93 файла).
- Верификация идентичности: diff kip8/index.html ↔ kip8test/index.html = ТЕ ЖЕ 82 строки репо-специфики, что и до переноса (коды задач совпали полностью, сдвиг только в номерах строк); scripts/WorkSchedule.gs ↔ kip8test — 0 различий.
- Тесты kip8: node tests/run-all.js = 3823 passed / 0 failed (было 3732/0; +91: 401 ×23, 402 ×27, 403 ×29, 404 ×12; адаптации 12 файлов). node --check GS OK; task401-syntax-check.py (4 inline-блока) OK.
- Браузер task401-404-browser-check.py 54/54 ×3 контекста (порт 9008; ключи localStorage kip8 БЕЗ префикса; собран из task404-browser-check + блок годовой шторки из task401-browser-check — scripts/task401-404-build-check.py): тёмная view — 401: ws-tt-yearfull, ширина шторки = рабочая область − ФИО (±4px), левый край = правый край ФИО (±3px), шапка двухстрочная «дней/часов» colspan 12; 402/403: «Слесарь КИПиА 5 р.»/«4 р.» без группы, тип третьей строкой, окно мероприятий — НЕТ «Отпуска ·» при живом отпуске (СИЗ/мероприятия живы), попап без СИЗ, «Группа допуска» в профиле; светлая edit — 404: ТРИ колонки (профиль+отпуска | СИЗ | мероприятия), tops в одну линию (±2px), СИЗ левее мероприятий, равные ширины, кнопки ✎/✕ рядом (зазор ≤12px, прижаты ≤22px), правка комментария → updateEmployee (группа отдельно — багфикс 403 жив), сводная с колонкой группы; мобайл 375 — стек профиль → отпуска → СИЗ → мероприятия, лента ярлыков; 0 JS-ошибок ×3. Пруфы task401-404k8-proof-*.png ×5. VLM ×1 (glm-5v-turbo): годовая шторка — «дней/часов» под месяцами, итоговые столбцы фоном+разделитель, сетка закрыта (примыкание/ширина — строгие DOM-замеры Y2/Y3).
- Деплой: коммит + push (PAT → сброс URL, leak-check 0); DEPLOY-Task401-404-transfer-tab-grid-card-events.md (⚠ СЕРВЕРНЫЙ ШАГ: обновить WorkSchedule.gs в Apps Script табель_КИП_ИОС — изменения 402/403: _accessGroupColIndex/_headerColIndex, listEmployees/updateEmployee/addEmployee; до деплоя — группа не читается/не сохраняется и баг комментария остаётся); эта worklog-запись.

Stage Summary:
- kip8 (SW kipia-v476) — перенесены Task 401+402+403+404 (код побайтово = kip8test, репо-специфика сохранена): годовая шторка итогов на всю ширину; тип работника третьей строкой + «группа_допуска» (карточка/сводная/шторка/GAS); «разряд» → «р.» и без группы в столбце ФИО; попап без СИЗ; GAS-фикс бага комментария (столбцы по заголовкам); ярлыки по самому длинному тексту; карточка — ТРИ колонки (СИЗ слева от мероприятий), кнопки ✎/✕ рядом; окно мероприятий без отпусков. Тесты 3823/0; браузер 54/54; VLM ×1. ⚠ СЕРВЕРНЫЙ ШАГ у пользователя: задеплоить WorkSchedule.gs в Apps Script. Следующий номер задачи: 405 (в обоих репо).
---
Task ID: 405+406 (перенос из kip8test)
Agent: Z.ai Code (kip8)
Task: Перенос Task 405 (разделение таблицы инструктажей: лист «Мероприятия», объединённый listTrainings, маршрутизация addTraining, сквозные id, trainingsSplitInit; блок «Повторные инструктажи и периодическая проверка знаний») и Task 406 (карточка: мероприятия ПОД отпусками; блоки СИЗ и инструктажей поменяны местами) из kip8test (106ee5a→0ef0fcd) в боевой kip8. Серверные шаги 405 пользователем уже выполнены (WorkSchedule.gs задеплоен, trainingsSplitInit запущен, лист «Мероприятия» создан).

Work Log:
- Базы сверены: kip8 = 4e7b0d5 (Task 401-404 + devices.json), kip8test = 0ef0fcd; тесты kip8 вне SW-версий совпадают с kip8test@a038911 (проверено фильтрующим diff по 12 файлам).
- scripts/task405-406-transfer.py: (1) task405-patch.py с cwd=kip8 — 17 правок GAS + 19 правок index.html чисто (относительные пути патча); (2) 5 правок Task 406 INLINE (task406-patch.py в kip8test с абсолютным путём — правки продублированы в скрипте переноса с относительным); (3) копия 14 контентно-изменённых тестов (tab-numbers/306/384/385/388/393/394/395/396/403/404/405/406/work-events) с маппингом kipia-test-v633→kipia-v476 (ассерты) и v634→v477 (guards; исторический guard kipia-test-v623 — общий, не тронут); kip8-специфичные файлы (test-task344 и др.) НЕ копировались; (4) run-all.js — require 405/406 после 404.
- scripts/task405-406-bump-sw.py: kipia-v476→v477 (двухшаговый: guards v477→v478, затем ассерты v476→v477; 95 файлов тестов; sw.js CACHE_VERSION).
- Тесты: 3863/0 (было 3823; +40 = дельта kip8test 3817→3857; kip8-специфичный test-task344 сохранён).
- Браузер scripts/task405-406-browser-check.py 41/41 ×3 (порт 9008, localStorage БЕЗ префикса): тёмная view — попап: «Мероприятия · год» = обучение+примечание, «Повторные инструктажи…» = инструктаж+ПЗ, без СИЗ, окно живо; светлая edit — карточка: колонка 1 = ТРИ окна (профиль → отпуска → МЕРОПРИЯТИЯ), колонка 2 = Повторные инструктажи, колонка 3 = СИЗ, верх в одну линию, ✎/✕ (4+4), префиллы типов, сводная 2/2; мобайл 375 — стек профиль → отпуска → мероприятия → инструктажи → СИЗ; 0 JS-ошибок ×3. Пруфы task405-406k8-*.png ×4. VLM ×1 (glm-5v-turbo): раскладка подтверждена.
- Верификация: WorkSchedule.gs — 0 различий с kip8test (побайтово); index.html — diff только репо-специфика (префиксы kip8test:/isolateLocalStorage/бекпорт-комментарии), контентных различий нет; sw.js/manifest — репо-специфика путей.
- Деплой: коммит + push (PAT → сброс URL); DEPLOY-Task405-406-transfer-instr-split-card-layout.md; эта worklog-запись.

Stage Summary:
- kip8 (SW kipia-v477) — Task 405+406 перенесены: разделение таблицы инструктажей (лист «Мероприятия» + серверная логика уже в Apps Script у пользователя) + раскладка карточки (мероприятия под отпусками, СИЗ ↔ инструктажи). Тесты 3863/0; браузер 41/41; VLM ×1. Серверных шагов НЕТ (GAS уже развёрнут). Оба сайта синхронизированы: kip8test v633 / kip8 v477. Следующий номер: 407 (в обоих репо).
