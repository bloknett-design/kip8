...
//     A. happy path: 14 дн. «О», перекрытие авто-смен, Сб/Вс, счётчики
//     B. период через границу месяца
//     C. идемпотентность: устаревшие «О» снимаются
...
describe('Task 279 — симуляция generateMonth (мок-таблицы)', () => {
    ...
    test('A: happy path — 14 дн. «О», перекрытие авто-смены, Сб/Вс, счётчики', () => {
        ...
        // перекрыться «О» на месте (ветка toUpdate)
        ...
        // все 14 дней отпуска — «О»
        let cntO = 0;
        for (let d = 10; d <= 23; d++) {
            if (byDate['2026-08-' + String(d).padStart(2, '0')] === 'О') cntO++;
        }
        assertEqual(cntO, 14, 'дней «О» внутри периода 10–23.08');

        // дни цикла 1/2 внутри отпуска (10,13,14,17,18,21,22) — смены не
        // проскочили, дни цикла 3/4 и Сб/Вс — вставка «О»; счётчики
        // (генерация ДО фиксы этот сценарий тоже проходила)
        assertEqual(res.data.vacationUpdated, 7, 'vacationUpdated (смена→«О»: 6 в toInsert + 1 в листе)');
        assertEqual(res.data.vacationGenerated, 7, 'vacationGenerated (пустые дни → вставка «О»)');
        ...
        assertEqual(sheets['Записи_графика'].rows[1][2], 'О', 'строка 2 (смена 10.08) стала «О»');
        assertEqual(iso(sheets['Записи_графика'].rows[1][0]), '2026-08-10', 'дата строки 2 не съехала');

        // Сб/Вс внутри отпуска (15–16.08.2026) — «О» (календарные дни)
        assertEqual(byDate['2026-08-15'], 'О', 'суббота 15.08 = «О»');
        assertEqual(byDate['2026-08-16'], 'О', 'воскресенье 16.08 = «О»');

        // «О» — источник «авто», без инструкции
        const oRows = es.filter(r => r[2] === 'О');
        assertTrue(oRows.every(r => r[5] === 'авто'), 'источник всех «О» — авто');
        assertTrue(oRows.every(r => r[8] === null || r[8] === ''), 'инструкция у «О» пуста');
    ...
    test('B: период через границу месяца 25.07–05.08 — обе половины', () => {
        ...
        const cntAug = entriesOf(sheets['Записи_графика'], '017')
            .filter(r => r[2] === 'О' && r[0].getMonth() === 7).length;
        assertEqual(cntAug, 5, 'август: «О» 01–05.08');
        WS.generateMonth({ token: 't', year: 2026, month: 7 });
        const cntJul = entriesOf(sheets['Записи_графика'], '017')
            .filter(r => r[2] === 'О' && r[0].getMonth() === 6).length;
        assertEqual(cntJul, 7, 'июль: «О» 25–31.07');
    });

    test('C: идемпотентность — период удалён, устаревшие «О» сняты', () => {
        const sheets = baseSheets();
        sheets['Отпуска'].rows.push([1, '017', 1, new Date(2026, 7, 10), new Date(2026, 7, 23), '']);
        const WS = loadWS(sheets);
        WS.generateMonth({ token: 't', year: 2026, month: 8 });
        const before = entriesOf(sheets['Записи_графика'], '017').filter(r => r[2] === 'О').length;
        assertEqual(before, 14, 'до удаления: 14 «О»');
        sheets['Отпуска'].rows.splice(1, 1);
        const res2 = WS.generateMonth({ token: 't', year: 2026, month: 8 });
        const after = entriesOf(sheets['Записи_графика'], '017').filter(r => r[2] === 'О').length;
        assertEqual(after, 0, 'после повторной генерации: 0 «О»');
    ...
    test('D (регресс Task 279): даты ТЕКСТОМ «10.08.2026» теперь работают', () => {
        ...
        const res = WS.generateMonth({ token: 't', year: 2026, month: 8 });
        const cnt = entriesOf(sheets['Записи_графика'], '017').filter(r => r[2] === 'О').length;
        assertEqual(cnt, 14, '«Сформировать» проставил 14 «О» (до Task 279 — 0)');
    });
    ...
    test('F: период через границу года 29.12.2026–11.01.2027', () => {
        ...
        const cnt = entriesOf(sheets['Записи_графика'], '017').filter(r => r[2] === 'О').length;
        assertEqual(cnt, 3, 'декабрь 2026: «О» 29–31.12');
    });
