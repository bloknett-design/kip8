...
#   F 0 JS-ошибок.
# kip8-ВАРИАНТ: localStorage БЕЗ префикса kip8test:.
# Порт 9005 (запуск: python3 -m http.server 9005 &).
...
def attach(page, ctx, theme, tag, viewer=False):
    ...
    ctx.add_init_script(
        ...
        "localStorage.setItem('kip8_session_token','bc-t391-%s');" % tag +
...
with sync_playwright() as p:
    ...
    page.screenshot(path='/tmp/t391k8-proof-workers-dark.png', full_page=False)
    ...
    page.screenshot(path='/tmp/t391k8-proof-add-sheet.png', full_page=False)
    ...
    check('D4: светлая — шрифт строк 14px / 600 / 0.95',
          ...
          wl['staffFont'])
    page3.screenshot(path='/tmp/t391k8-proof-workers-light.png', full_page=False)
    ...
    check('E2: мобайл — кнопка ОРАНЖЕВАЯ #C6613F в шапке, текст белый',
          ...
          wm['btn'])
    page2.screenshot(path='/tmp/t391k8-proof-mobile-workers.png', full_page=False)
