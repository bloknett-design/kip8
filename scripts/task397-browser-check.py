...
#      общего правила).
# Порт 9007 (запуск: python3 -m http.server 9007 &); kip8 — localStorage БЕЗ префикса kip8test:.
import json
...
def attach(page, ctx, theme, tag, perms, role='Админ'):
    ...
    def block_external(route):
        route.fulfill(status=404, content_type='text/plain',
                      body='not found (t397k8-%s)' % tag)
    ctx.route('**raw.githubusercontent.com/**', block_external)
    ctx.route('**calendar.legalic.ru/**', block_external)
    ctx.add_init_script(
        "try{localStorage.removeItem('kip8_my_access')}catch(e){};" +
        "try{localStorage.removeItem('kip8test:kip8_my_access')}catch(e){};" +
        "localStorage.setItem('kip8_session_token','bc-t397k8-%s');" % tag +
...
with sync_playwright() as p:
    ...
    page.screenshot(path='task397k8-proof-zapret.png', full_page=False)
    ctx.close()
    ...
    page.screenshot(path='task397k8-proof-admin.png', full_page=False)
    ctx.close()
    ...
    page.screenshot(path='task397k8-proof-mobile.png', full_page=False)
    ...
    page.screenshot(path='task397k8-proof-mobile-click.png', full_page=False)
