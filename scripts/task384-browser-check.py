...
# Порт 8995 (БОЕВОЙ kip8 — перенос из kip8test c0ed8d7;
# localStorage БЕЗ префикса — урок Task 365).
...
def attach(page, ctx, theme, tag, viewer=False):
    ...
    def block_external(route):
        route.fulfill(status=404, content_type='text/plain',
                      body='not found (t384k8-%s)' % tag)
    ctx.route('**raw.githubusercontent.com/**', block_external)
    ctx.route('**calendar.legalic.ru/**', block_external)
    ctx.add_init_script(
        "try{localStorage.removeItem('kip8_ws_cache_v1')}catch(e){};" +
        "localStorage.setItem('kip8_session_token','bc-t384k8-%s');" % tag +
...
with sync_playwright() as p:
    ...
    page.screenshot(path='/tmp/t384k8-proof-card.png', full_page=False)
    ...
    page.screenshot(path='/tmp/t384k8-proof-emp-edit-sheet.png', full_page=False)
    ...
    page.screenshot(path='/tmp/t384k8-proof-vac-edit-sheet.png', full_page=False)
    ...
    page3.screenshot(path='/tmp/t384k8-proof-mobile-sheet.png', full_page=False)
