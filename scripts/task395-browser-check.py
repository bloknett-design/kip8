...
def attach(page, ctx, theme, tag, perms, role='Админ'):
    ...
    ctx.add_init_script(
        ...
        "localStorage.setItem('kip8_session_token','bc-t395k8-%s');" % tag +
...
with sync_playwright() as p:
    ...
    page.screenshot(path='task395k8-proof-workers-dark.png', full_page=False)
    ...
    page2.screenshot(path='task395k8-proof-workers-view.png', full_page=False)
    ...
    check('J3: светлая — верхняя кромка СВЕТЛЕЕ боков (блик)',
          ...
          btop2[1] > bside2[1],
          (m['btop'], m['bside']))
    page5.screenshot(path='task395k8-proof-mobile-light.png', full_page=False)
