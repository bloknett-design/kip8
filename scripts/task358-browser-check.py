...
#      (localStorage), баннер «ждут отправки: 1».
...
def mock_response(action):
    ...
    if action == 'verifyOTP':
        return {'ok': True, 'data': {'token': 't358-desk8', 'role': ROLE,
...
def route_all(ctx):
    ...
    def block_external(route):
        route.fulfill(status=404, content_type='text/plain', body='not found (t358k)')
...
def main():
    ...
    with sync_playwright() as p:
        ...
        ui_login(page)
        check('вход выполнен', page.evaluate("localStorage.getItem('kip8_session_token')") == 't358-desk8')
        ...
        if upd_beacon:
            check('beacon: payload содержит токен и curr=96',
                  upd_beacon[-1]['payload'].get('token') == 't358-desk8' and
