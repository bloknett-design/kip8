#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Task 365 (перенос из kip8test 4a22d39): browser-check в kip8 — заявка
# пользователя (с поправкой):
...
# Отличие от kip8test-версии: localStorage БЕЗ префикса изоляции
# (kip8 — основной репозиторий, сырые ключи kip8_session_token /
# app-theme / kip8_flow_cache_v1), порт 8964.
# Контексты:
...
with sync_playwright() as p:
    ...
    def block_external(route):
        route.fulfill(status=404, content_type='text/plain', body='not found (browser-check t365 kip8)')
    ...
    # kip8 — БЕЗ изоляции: сырые ключи приложения.
    ctx.add_init_script(
        "localStorage.setItem('kip8_session_token','bc-t365k-a');" +
        "localStorage.setItem('app-theme','dark');" +
        "localStorage.setItem('kip8_flow_cache_v1', JSON.stringify(%s));" % cache_seed())
    ...
    ctx2.add_init_script(
        "localStorage.setItem('kip8_session_token','bc-t365k-b');" +
        "localStorage.setItem('app-theme','dark');" +
        "localStorage.setItem('kip8_flow_cache_v1', JSON.stringify(%s));" % cache_seed())
