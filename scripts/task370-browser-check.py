#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Task 370 (kip8, синхронно с kip8test): browser-check — заявка:
...
#   (изоляция kip8test: есть только в тестовом репо).
import datetime
import json
import sys
...
with sync_playwright() as p:
    ...
    def block_external(route):
        route.fulfill(status=404, content_type='text/plain', body='not found (browser-check t370-k8)')
    ctx.route('**raw.githubusercontent.com/**', block_external)
    ctx.route('**calendar.legalic.ru/**', block_external)

    # kip8: localStorage БЕЗ префикса (изоляция только в kip8test)
    ctx.add_init_script(
        "localStorage.setItem('kip8_session_token','bc-t370-k8-a');" +
    ...
    ctx3.add_init_script(
        "localStorage.setItem('kip8_session_token','bc-t370-k8-c');" +
        "localStorage.setItem('app-theme','light');" +
        "localStorage.setItem('kip8_flow_outbox_v1', %s);" % json.dumps(
    ...
    ctx2.add_init_script(
        "localStorage.setItem('kip8_session_token','bc-t370-k8-b');" +
        "localStorage.setItem('app-theme','dark');" +
        "localStorage.setItem('kip8_flow_outbox_v1', %s);" % json.dumps(
