...
# БЕЗ префикса kip8test: (перенос из kip8test@35c68c2, паттерн task398k8).
import datetime
import json
...
def attach(page, ctx, theme, tag, perms, role):
    ...
    ctx.add_init_script(
        ...
        "localStorage.setItem('kip8_session_token','bc-t399k8-%s');" % tag +
...
with sync_playwright() as p:
    ...
    page.screenshot(path='task399k8-proof-min.png', full_page=False)
    ...
    page.screenshot(path='task399k8-proof-min-day.png', full_page=False)
    ...
    page.screenshot(path='task399k8-proof-view.png', full_page=False)
    ...
    page.screenshot(path='task399k8-proof-mobile.png', full_page=False)
