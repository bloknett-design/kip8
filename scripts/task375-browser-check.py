...
def setup_ctx(browser, viewport, token, theme='dark'):
    ...
    def block_external(route):
        route.fulfill(status=404, content_type='text/plain', body='not found (browser-check t375-k8)')
    ctx.route('**raw.githubusercontent.com/**', block_external)
    ctx.route('**calendar.legalic.ru/**', block_external)
    ctx.add_init_script(
        "localStorage.setItem('kip8_session_token','%s');" % token +
...
with sync_playwright() as p:
    ...
    ctx, page, js_errors = setup_ctx(browser, {'width': 1280, 'height': 800}, 'bc-t375k8-a')
    ...
    ctx, page, js_errors = setup_ctx(browser, {'width': 1280, 'height': 800}, 'bc-t375k8-b')
    ...
    ctx, page, js_errors = setup_ctx(browser, {'width': 375, 'height': 760}, 'bc-t375k8-c')
    page.evaluate("navigateTo('work-schedule')")
    page.wait_for_timeout(3000)
    page.evaluate(STUB_PRINT)
    page.click('#wsPrintBtn')
