#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Task 359: browser-check (kip8, прод) — красный «пора вводить» ЯРЧЕ + шрифт крупнее.
#
...
def mock_response(action):
    ...
    if action == 'verifyOTP':
        return {'ok': True, 'data': {'token': 't359p-desk', 'role': ROLE,
...
def run_theme(browser, theme):
    ...
    page.add_init_script("try{localStorage.setItem('app-theme','%s')}catch(e){}" % theme)
