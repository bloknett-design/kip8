...
#   M4 0 JS-ошибок.
# Порт 9007 (запуск: python3 -m http.server 9007 &); localStorage БЕЗ
# префикса kip8test: (боевой репо).
import calendar
import datetime
import json
...
def attach(page, ctx, theme, tag):
    ...
    def block_external(route):
        route.fulfill(status=404, content_type='text/plain',
                      body='not found (t394k8-%s)' % tag)
    ctx.route('**raw.githubusercontent.com/**', block_external)
    ctx.route('**calendar.legalic.ru/**', block_external)
    ctx.add_init_script(
        "try{localStorage.removeItem('kip8_ws_cache_v1')}catch(e){};" +
        "try{localStorage.removeItem('kip8test:kip8_ws_cache_v1')}catch(e){};" +
        "localStorage.setItem('kip8_session_token','bc-t394k8-%s');" % tag +
