...
# Порт 9007 (запуск: python3 -m http.server 9007 &); kip8 — localStorage
# БЕЗ префикса kip8test: (перенос из kip8test@90a024c, паттерн task397k8).
import calendar
import datetime
import json
...
def attach(page, ctx, theme, tag, perms, role):
    ...
    ctx.add_init_script(
        ...
        "localStorage.setItem('kip8_session_token','bc-t398k8-%s');" % tag +
...
with sync_playwright() as p:
    ...
    check('F: DOM-скан табеля: 0 визуально-протекающих [hidden]-элементов',
          leaks == [], leaks)
    page.screenshot(path='task398k8-proof-min.png', full_page=False)
    ...
    page.screenshot(path='task398k8-proof-view-toolbar.png', full_page=False)
    ...
    page.screenshot(path='task398k8-proof-view.png', full_page=False)
    ...
    page.screenshot(path='task398k8-proof-mobile.png', full_page=False)
