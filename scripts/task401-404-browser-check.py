...
def attach(page, ctx, theme, tag):
    ...
    ctx.add_init_script(
        ...
        "try{localStorage.removeItem('kip8_my_access')}catch(e){};" +
        "localStorage.setItem('kip8_session_token','bc-t401-404k8-%s');" % tag +
...
with sync_playwright() as p:
    ...
    # 2) Task 401: годовая шторка итогов — на всю ширину правее ФИО
    ...
    page.screenshot(path='task401-404k8-proof-grid-dark.png', full_page=False)
    ...
    check('F10: СИЗ (образец) — ✎ и ✕ рядом',
          ...
          c2 and c2['ppe'])
    page.screenshot(path='task401-404k8-proof-workers-light.png', full_page=False)
    ...
    page.screenshot(path='task401-404k8-proof-comment-edit.png', full_page=False)
    ...
    page.screenshot(path='task401-404k8-proof-mobile.png', full_page=False)
