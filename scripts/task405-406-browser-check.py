#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Task 405-406-transfer: browser-check БОЕВОГО kip8 (порт 9008,
# localStorage БЕЗ префикса — репо kip8). Заявки: 405 — разделение
# таблицы инструктажей; 406 — блок мероприятия под блоком отпуска,
# блоки СИЗ и инструктажей поменяны местами.
...
# Порт 9008 (запуск: python3 -m http.server 9008 &).
import datetime
import json
...
def attach(page, ctx, theme, tag):
    ...
    ctx.add_init_script(
        ...
        "try{localStorage.removeItem('kip8_my_access')}catch(e){};" +
        "localStorage.setItem('kip8_session_token','bc-t405-406k8-%s');" % tag +
...
with sync_playwright() as p:
    ...
    check('C8: в попапе НЕТ секции СИЗ (регресс Task 403)',
          'СИЗ' not in card)
    page.screenshot(path='task405-406k8-proof-grid-dark.png', full_page=False)
    ...
    page.screenshot(path='task405-406k8-proof-workers-light.png', full_page=False)
    ...
    page.screenshot(path='task405-406k8-proof-instr-form.png', full_page=False)
    page.evaluate("WorkSchedule.closeTrainingForm()")
    page.wait_for_timeout(400)
    ...
    page.screenshot(path='task405-406k8-proof-mobile.png', full_page=False)
