...
def attach(page, ctx, theme, tag):
    ...
    def block_external(route):
        route.fulfill(status=404, content_type='text/plain', body='not found (t382k8-%s)' % tag)
    ctx.route('**raw.githubusercontent.com/**', block_external)
    ctx.route('**calendar.legalic.ru/**', block_external)
    ctx.add_init_script(
        "localStorage.setItem('kip8_session_token','bc-t382');" +
    ...
    for theme in ('dark', 'light'):
        ...
        if 'err' in r:
            ...
            if theme == 'light':
                ...
                page.screenshot(path='/tmp/t382k8-mobile-light.png', full_page=False)
                img = Image.open('/tmp/t382k8-mobile-light.png').convert('RGB')
